...
const rows = readdirSync(raw).filter((f) => f.startsWith('peritext-') && f.endsWith('.json'))
  ...
  .filter((r) => r.config?.scenario && r.config?.matrixVersion === 3);
for (const key of new Set(rows.map((r) => `${r.system}:${r.workload}:${r.preset}`))) {
  const rs = rows.filter((r) => `${r.system}:${r.workload}:${r.preset}` === key);
