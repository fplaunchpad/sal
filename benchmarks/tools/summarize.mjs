...
L.push('Recovery is the median of five runs that load the system\'s primary native artifact into a fresh document and materialize its visible text.');
L.push('');
L.push('EmbedRGA and SidedEmbedRGA are both first-class verified designs. EmbedRGA targets lower retained metadata; SidedEmbedRGA retains additional ordering-policy state to prevent the documented interleaving anomaly. The tables present both so applications can choose that space/semantics tradeoff. RGA is the verified explicit-tombstone baseline.');
...
for (const t of traces) {
  ...
  L.push('');
  L.push('*Recovery loads the primary native artifact into a fresh document and materializes visible text (median of five runs). Artifacts retain different information; compare only rows with compatible `contains` descriptions below. JS heap/RSS are intentionally excluded from cross-system ranking.');
