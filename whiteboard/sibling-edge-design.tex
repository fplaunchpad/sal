...
concurrent runs stay contiguous (non-interleaving). The paths are ghost state, as
in RGA; the honest boundary (the tombstoned-oracle / strong-list fooling pair
remains impossible) is unchanged. Next step: the Lean port as a conditioned
instance extending \texttt{RGA\_Tombstone\_Free}.
\end{abstract}
...
\section{The paths are ghost state; the honest boundary}

\paragraph{Cost.} The spines $\pL,\pR$ are the analogue of RGA's carried path.
They are consulted only to place the ghost of a node deleted while still
referenced; under causal stability (every replica has seen the delete, no future
insert can reference the node) a ghost id can be dropped from all spines, so the
metadata is bounded by the in-flight deletes, not by history. This is the
\emph{tombstone-free-with-transient-ghosts} regime --- no permanent graves, but
per-deletion memory until stability, exactly as every published order-preserving
sequence CRDT pays somewhere (design record \S8).
...
\begin{enumerate}
  ...
  \item Single-replica commutation VC conditioned on \emph{accurate} spines
        (the two-sided analogue of RGA's \texttt{commutes\_with'} / accuracy),
        discharged through the conditioned framework; the spine is ghost state.
