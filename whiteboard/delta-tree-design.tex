...
live node's birth chain passes through them, and pruning the unreachable
rest at every merge is machine-checked sound (\S\ref{sec:validation}) ---
the ledger is a garbage-collected \emph{heap} (roots = live nodes,
pointers = birth parents), not a tombstone set. Reads never consult
$\Lambda$; merges never order by $\rho$.
...
\paragraph{The operation payload and replay rule (per the proved RGA).}
Following the \emph{why-the-path-matters} analysis: $\mathsf{rc}$ must be
...
delta at mint time. A reordered application resolves the id-path to the
nearest live ancestor and re-relativizes the carried absolute, landing
exactly where insert-then-delete would have put it: this fold-invariance
is machine-checked as \emph{exact state equality} (1303 reordered
ins$\parallel$del-anchor diamonds, \texttt{litmus/replay.py}). The
placement alone is not the whole replay rule, however: a 3-op
countermodel (CE1 in \texttt{replay.py}) shows that carried-slice
placement, though it commutes perfectly, can converge to a sibling order
contradicting the ledger's --- geometry minted against one replica's
sibling set is not canonical against another's. The correct rule
(machine-checked clean across lockstep runs, all countermodels, and 48/48
linearizations) adds a \emph{lazy canonical repair}: after any placement
or fold, if the touched level's geometric order contradicts the ledger's
birth-chain order, re-render \textbf{that level} with the sequential
carve. Invariant 1 of \S\ref{sec:merge} thus applies at op application
too, not only at merges. At the origin the repair never fires, so
generation remains exactly the local carve; concurrent same-anchor
inserts commute up to $\approx$ (read- and merge-future-equal).
...
\paragraph{Ledger retention, state size, and precision
(\texttt{litmus/measure.py}).} Three machine-checked facts fix the cost
model. \emph{(a) Pruning is referential, not causal.} Per-replica
reachability pruning --- at every merge, drop $\Lambda$-entries on no
live node's birth chain --- is \textbf{sound}: lockstep-clean against
unpruned across 800 DAG executions plus 210 churn executions (185{,}228
ops, 61{,}561 merges), with a deliberately-broken control confirming the
harness bites. The invariant is inductive: every live node's full birth
chain lies in $\Lambda$; at a merge, each survivor is live in some input,
and that input's own closure re-supplies exactly the chain entries the
survivor needs (entries are write-once, so unions never conflict).
Conversely, \textbf{causal stability alone is unsound}: a six-step
countermodel (\texttt{ins}\,1; \texttt{ins}\,2\,{\leftarrow}\,1;
$\del(1)$; full sync --- the delete now stable everywhere; drop
$\Lambda[1]$; \texttt{ins}\,3\,{\leftarrow}\,2; merge) breaks the chain
walk: survivor $2$ is live and its chain passes through the stably
deleted $1$. The ledger inverts the classical tombstone-GC picture: what
licenses forgetting is unreachability, not stability. \emph{(b) Size.}
The unpruned ledger equals the tombstoned RGA's state \emph{exactly}
(both $=$ \#inserts); the pruned ledger is $O(\mathrm{live})$: at round
300 of the churn regime, 9 live $+$ 17 ledger entries vs.\ 906 for
RGA$_\dagger$ ($\approx 35\times$, growing linearly). \emph{(c)
Precision.} Post-merge, the maximum denominator obeys an exact law ---
$\mathrm{bits} = \text{max sibling-set width} + 2$, verified on
3{,}241/3{,}241 merges --- because the re-render replays quarter-carves:
merges reset fold-accumulated precision, leaving a structural, not
historical, floor (churn: 14 bits across 1{,}803 ops, vs.\ 76 bits
merge-free).
...
position-invariant by construction). The width$+2$ precision law
(\S\ref{sec:validation}) says the fractions are bounded by structure,
not history --- so the mechanization can treat them as \emph{derived}
data (the render of the finite sibling order), never as stored truth.
\item \textbf{Garbage collection as heap reachability.} The pruning
obligation is referential --- live nodes are roots, birth parents are
pointers --- so it phrases naturally as separation-logic-style
reachability over the id heap, \emph{not} as a causal-stability
side-condition (which is machine-refuted, \S\ref{sec:validation}).
...
\item \textbf{The op layer.} \texttt{do\_} = carried-slice placement +
lazy canonical repair of the touched level (\S\ref{sec:design}), written
with the repair as the first-class case (empirically it is the common
path in replays). Its invariant is level-local --- \emph{every sibling
level canonical with respect to the ledger, and contained} --- with three
lemma-shaped obligations: placement$+$repair preserves it; fold$+$repair
preserves it; any invariant state reads exactly the canonical order of
(live ids, ledger). Replay $\approx$ merge is then immediate, since the
merge renders that same order.
...
(honest delivery; born accuracy + applicable delivery). Two scope facts
from the simulation sketch (\texttt{litmus/equiv-proof-sketch.md}): the
equivalence theorem is a statement of the \emph{MRDT model} (under
op-based causal delivery with tombstone-displayed anchors the two designs
genuinely separate), and no-resurrection at merges rests on the honest-LCA
/ reachability conditioning --- exactly the framework's existing
machinery.
\end{itemize}
