...
$\delta$. \emph{Any} order-preserving prefix-free code works here,
and the artifact exists twice to prove it: \code{embed-tree} carries
unary codes, \code{embed-code} the delta code above --- two encodings
of one datatype. The delta code is frugal but not the last word:
only the \emph{value} bill is forced at $\log_2 \delta$; the
\emph{length} bill can itself be entropy-coded --- the
order-preserving flip of Elias~$\delta$ (apply $C$ to the length
field, then the payload) is prefix-free and monotone by the same
across-length-class argument, at $\log_2 \delta + O(\log \log
\delta)$ per level. This is now the \emph{third proved}
\code{OrderedPrefixCode} instance (\code{eliasDeltaCode}), and the
capstone theorem is inherited at it verbatim, with zero new proof
content (\S\ref{sec:mechanization}) --- the code-parametricity claim,
machine-validated. Two honest footnotes: the flip wins only from
$\delta \ge 32$ and loses at $\delta \in \{2,3\} \cup [8,15]$
(kernel-checked length comparisons), and measured editing tails are
thin, so on real traces the two codes are a wash --- the upgrade's
value is the theorem, not the bits.
...
\begin{tabular}{@{}lccc@{}}
\toprule
scenario & \shortstack[c]{quarter carve\\ (ts-oblivious, refuted)} &
  unary mint & \textbf{delta-coded mint} \\
\midrule
chain then delete-all ($\delta{=}1$) & $2^{2001}$ &
  $2^{502501}$ ($\sim$61 KB) & $\boldsymbol{2^{4001}}$ ($\sim$0.5 KB) \\
1000 root siblings ($\delta{=}t$) & --- (ties) &
  max $2^{1003}$, 125 KB &
  \textbf{max $\boldsymbol{2^{23}}$, 5 KB} \\
\bottomrule
\end{tabular}
\end{center}
