...
three-way merges with an LCA) built from one idea: an element's sort
key is its \emph{birth chain} --- the sequence of timestamps from the
root of the birth tree down to the element, dead ancestors included ---
written at birth and never edited. Display order is lexicographic
order on birth chains (per level: newest first, RGA's rule); deletion
removes the element's record and nothing else; the merge is OR-set
survival over event sets, contains no repair, and never decides an
order. Timestamps are unique, so \emph{ties cannot exist}; chains
travel with the insert events, so the state is a canonical function of
the event set. The whole state is the live nodes --- no tombstones, no
ledger, no event log: the dead persist only as entries inside their
descendants' sort keys. On top of this sits a separable
\emph{representation} layer: chains are carried as parent-relative
dyadic intervals through an order-preserving prefix-free code of the
timestamp \emph{differentials}, which realizes chain concatenation as
affine composition, turns deletion into an exact isometric fold, and
prices a dead ancestor at $\Theta(\log \delta)$ coordinate bits ---
the entropy of the race that created it, the information-theoretic
minimum, paid only at genuine races. The design is machine-validated:
clean on the L1--L25 anomaly battery (except backward-run interleaving
L19, the published RGA's own anomaly) and on 420+ randomized
version-DAG executions, and \textbf{lockstep read-equal with the
published tombstoned RGA at every step of 120 randomized executions}.
Insert operations carry the anchor's birth chain --- consulted by no
reachable execution, kept because the proof of $\rc = \Either$
commutation requires totality of reordered application; the prefix is
ghost state in the strict, machine-checkable sense. We give the
datatype and its one-line metatheory, the encoding and its
faithfulness theorem, the validation record, a detailed comparison
with Eg-walker, related work, and the Lean mechanization plan on the
proved RGA scaffolding.
\end{abstract}
