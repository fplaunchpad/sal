...
- [`Sal/MRDTs/Metatheory/`](Sal/MRDTs/Metatheory) — the ternary (three-way merge) metatheorem over the version DAG: the LCA lemma as a reachability invariant, a validated VC set (`CoreVCs3CD + FeasibleDeltaVCs3 + CDVC3`), and kernel-checked end-to-end RA-linearizability for the production **OR-Set, OR-Set-efficient, and Enable-wins flag** (plus Counter and G-Set), all discharges in a single `MRDT_Instances.lean`. Superseded routes, impossibility results, and plans live under `Development/`.
