...
- [`Sal/MRDTs/Metatheory/`](Sal/MRDTs/Metatheory) — the ternary (three-way merge) metatheorem over the version DAG: the LCA lemma as a reachability invariant, a validated VC set (`CoreVCs3CD + FeasibleDeltaVCs3 + CDVC3`), and kernel-checked end-to-end RA-linearizability for **all 12 production MRDTs** through one generic conditioned framework: the eleven flat datatypes (OR-Set, OR-Set-efficient, Enable-wins flag, Grow-Only Set and Map, Increment-Only and PN counters, tombstone RGA, Peritext, Multi-Valued Register, Add-Wins Priority Queue) at the identity instantiation with their VC discharges in `MRDT_Instances.lean`, and the **tombstone-free RGA** — provably outside the VC schema — at full generality (`RGA_TombstoneFree_RA_Lin.lean`): RA-linearizability up to observational `≈` at every reachable configuration, from a single honest-delivery assumption (born accuracy + applicable delivery), kernel-clean. Superseded routes, impossibility results, and plans live under `Development/`.
