...
- **Plausible** generates concrete counterexamples for decidable VCs. The canonical demo is the enable-wins flag MRDT, which contains a known bug from prior work (the `inter_right_1op` VC fails); Plausible rediscovers a minimal failing execution automatically. See [`CaseStudies/Fstar_like_implementations/MRDTs/SAL/en_wins_flag.lean`](CaseStudies/Fstar_like_implementations/MRDTs/SAL/en_wins_flag.lean).
