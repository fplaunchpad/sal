...
- `RGA_Tombstone_Free` — the **tombstone-free, path-carrying RGA**: deletes really remove (no tombstone set); each op carries its target's recorded ancestor path, and merge rehomes survivors along it. Provably outside the standard 24-VC schema (commutation is reachability-conditioned, and a prefix-free variant is impossible — `RGA_PrefixFree_Impossible.lean`); verified instead by a direct end-to-end theorem, `rga_tombstone_free_ra_linearizable3_eq`: RA-linearizability up to observational `≈` at every reachable configuration under honest delivery, via the applicability-conditioned metatheory ([`Sal/MRDTs/Metatheory/RGA_TombstoneFree_RA_Lin.lean`](Sal/MRDTs/Metatheory/RGA_TombstoneFree_RA_Lin.lean)). See [`Sal/MRDTs/RGA_Tombstone_Free/doc/why-the-path-matters.pdf`](Sal/MRDTs/RGA_Tombstone_Free/doc/why-the-path-matters.pdf).
...
- [`docs/metatheory-note/mrdt-metatheory-note.pdf`](docs/metatheory-note/mrdt-metatheory-note.pdf) — a self-contained paper-style note on the MRDT (ternary-merge) metatheory: the eight VCs with intuition, the delta contract and why no lattice contract can exist, the causal-delta equation, the LCA lemma erratum, and the discharged production catalogue, with TikZ execution diagrams. Its open questions carry kernel-checked findings from the conditioned-metatheory investigation (`Development/`): the defeater defeats even the commuting `inter_lca_2op` rule, the naive route-reunification is refuted (→ a closure-indexed contract), and the naive feasible-update conditioning is refuted (→ applicability-aware convergence).
...
- [`Sal/MRDTs/Metatheory/`](Sal/MRDTs/Metatheory) — the ternary (three-way merge) metatheorem over the version DAG: the LCA lemma as a reachability invariant, a validated VC set (`CoreVCs3CD + FeasibleDeltaVCs3 + CDVC3`), and kernel-checked end-to-end RA-linearizability for **9 of the 12 production MRDTs** (OR-Set, OR-Set-efficient, Enable-wins flag, Grow-Only Set and Map, Increment-Only and PN counters, RGA, Peritext), all discharges in a single `MRDT_Instances.lean`. The **tombstone-free RGA** — provably outside the VC schema — carries a direct end-to-end theorem via the applicability-conditioned metatheory (`RGA_TombstoneFree_RA_Lin.lean`): RA-linearizability up to observational `≈` at every reachable configuration, from a single honest-delivery assumption (born accuracy + applicable delivery), kernel-clean. The remaining two, MVR and the Add-Wins Priority Queue, are class-placed with mechanical recipes. Superseded routes, impossibility results, and plans live under `Development/`.
