...
- [`Sal/CRDTs/`](Sal/CRDTs) — state-based CRDTs in the `⟨Σ, σ₀, do, merge, rc⟩` signature. Contains the paper's 4 CRDTs (`Increment_Only_Counter_CRDT`, `Multi_Valued_Register_CRDT`, `OR_Set_CRDT`, `PN_Counter_CRDT`) together with 13 additional post-paper CRDTs: LWW / MAX / MIN registers, LWW Element Set, LWW Map, MAX Map, Grow-Only Set / Multiset, Shopping Cart, insert-only Priority Queue, an Add-Win CRPQ adapted from Zhang et al. 2023, a Bounded Counter (Sypytkowski 2019 / Balegas et al. 2015 — PN-counter plus a sparse per-replica-pair `transfers` map for quota redistribution), and a state-based RGA (Replicated Growable Array — the classic sequence CRDT underlying Automerge / Yjs, here in its state-based formulation as a grow-only `Map OpId (char, afterId, deleted)`). Git history (`papoc2026` branch) distinguishes paper-era from post-paper files.
