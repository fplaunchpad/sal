...
- **Shesha** (`Sal/ConditionedMRDTs/MRDT_Instances/Shesha/`) — a **tombstone-free sibling-linked replicated list** (order lives in links between live nodes; delete splices and retains *nothing*), designed live on the whiteboard (2026-07) and named for Ananta-Shesha, "the remainder". Phase-1 verification: `sequential_soundness` (single-replica folds = the naive linked list; kernel-clean) and `delete_preserves_survivor_order` — the property the flat tombstone-free RGA provably violates — plus machine-checked litmus witnesses byte-matched to the Python reference, merge symmetry (`merge_comm`) proved at state equality, and the two **fooling-pair impossibility theorems** (`I1/I2_no_merge_function`: no tombstone-free merge can match the tombstoned oracle, nor the strong list spec) quantified over all merge functions. Phase-2b merge lemmas (`Shesha_Forest/Skel/M0.lean`: a parent-chain/depth layer, the exact skeleton characterization, and a placed-exactly-once accounting): **Lemma M0's well-formedness closed kernel-clean** — `merge_WF` (`merge_read_nodup`: the merge places every id at most once, via a generic DFS-count engine over splice-chain determinism) and `merge_mem_liveM` (the merge displays only merge-live ids). Remaining diagnostic sorries: 2 — `merge_ids`' coverage half (every liveM id *is* placed) and `merge_L_filter` (the M2 order-extension core); convergence VCs and Theorem P (causal pairwise display stability) pending. Design record, pen-and-paper proofs, PBT harness (~16k merges, 0 displayed-pair flips) and the **verified anomaly-comparison matrix** (6 implementations + yjs/automerge/loro/list-positions) under [`whiteboard/`](whiteboard/).
