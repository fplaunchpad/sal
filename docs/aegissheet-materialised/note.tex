...
Lean port & definitions machine-checked; theorems stated & \lean{AegisSheetMaterialised.lean}: signature, canonical state, honesty predicate, and theorem statements compile with no \lean{sorry}; proofs owed \\
...
Conjecture H1 and the port's theorems & \lean{JoinTarget}, \lean{UpdatePreservesCanon}, \lean{ObservationEquivalence}, \lean{Honest} & stated in Lean as \lean{Prop}-valued definitions, unproved; validated on the campaign scope \\
...
\item Prove the stated theorems of \lean{AegisSheetMaterialised.lean}:
\lean{UpdatePreservesCanon}, the merge counterpart, \lean{JoinTarget} by the
queue's route (fold canonicity under \lean{Honest}, then set algebra), and
