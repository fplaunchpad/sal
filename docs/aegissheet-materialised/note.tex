...
\midrule
H1, named removal & machine-checked for sequential honest histories; validated on concurrent executions; conjectured for concurrent executions in general & \lean{Reach.materialised} with \lean{observationEquivalence}: the materialised fold of the erased operations of every union-model history reached by applicable insertions observes exactly the union model's purge-free view. No counterexample in 3000 concurrent executions with Lamport clocks, laggard replicas, and purge markers: 110{,}560 versions, 19{,}240 registered-ancestor and 461 virtual-base merges, three seeds; all 90{,}859 generated events pass a transcribed \lean{applicableB} after erasing $\mathit{kills}$ \\
...
Lean port & Join, replay adequacy, and observation equivalence machine-checked & \lean{JoinOn M Honest}, \lean{ReplayAdequacyCertificate M generation}, \lean{ObservationEquivalence}, and \lean{UpdatePreservesCanon} are proved (no \lean{sorry}, standard axioms only); the fold over concurrent honest enumerations, the sequential certificate, and the \lean{VerifiedMRDT} package are owed \\
...
Update preservation & \lean{AegisSheetMaterialisedUpdate.lean}: \lean{update\_preserves\_canon}, \lean{updatePreservesCanon}, \lean{HonestHistory.insert}, \lean{Reach.materialised} & machine-checked: $\mathsf{do}(\mathit{canon}(E), \mathit{toM}(E,e)) = \mathit{canon}(E \cup \{e\})$ for every honest history $E$ and event $e$ applicable at $E$; honesty is preserved; the materialised fold of every reachable history is its canonical state; same axioms \\
...
What remains for H1 on concurrent executions is the fold over honest
enumerations: for an enumeration $\rho$ of $E$ in which each event's causal
summary is a subset of the events before it, and each removal's
$\mathit{kills}$ are the live tokens of the issuer's summary rather than of
the whole prefix, the materialised fold of $\rho$ equals $\mathit{canon}(E)$.
The restricted Join already gives that every reachable materialised state
is such a fold. The sequential certificate needs a representation relation
through \lean{canon}; \lean{no\_view\_only\_step} shows the visible sheet is
insufficient, and \lean{row\_tokens\_distinguish\_future},
...
\lean{ObservationEquivalence}, and \lean{UpdatePreservesCanon} are proved.
Remaining: the fold over concurrent honest enumerations; then the sequential
certificate through \lean{canon} and the \lean{VerifiedMRDT} package; keep
the union model until the runtime measurement is in.
