...
Lean port & Join machine-checked; equivalence stated & \lean{JoinOn M Honest} is proved (\lean{joinTarget}, no \lean{sorry}, standard axioms only); \lean{UpdatePreservesCanon} and \lean{ObservationEquivalence} remain statements; the sequential certificate and the \lean{VerifiedMRDT} package are owed \\
...
\item \lean{JoinTarget} is proved. Remaining: discharge \lean{Honest} from
\lean{MintHonest} of an issuance predicate (\lean{IssuanceEstablishes}) to
obtain the replay-adequacy certificate; prove \lean{UpdatePreservesCanon} and
\lean{ObservationEquivalence}; then the sequential certificate and the
