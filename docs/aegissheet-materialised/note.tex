...
Lean port & Join, replay adequacy, and observation equivalence machine-checked & \lean{JoinOn M Honest}, \lean{ReplayAdequacyCertificate M generation}, and \lean{ObservationEquivalence} are proved (no \lean{sorry}, standard axioms only); \lean{UpdatePreservesCanon} remains a statement; the sequential certificate and the \lean{VerifiedMRDT} package are owed \\
...
Update preservation & \lean{UpdatePreservesCanon} & stated in Lean, unproved; validated on the campaign scope \\
...
\item \lean{JoinTarget}, the replay-adequacy certificate, and
\lean{ObservationEquivalence} are proved. Remaining:
\lean{UpdatePreservesCanon}; then the sequential certificate through
\lean{canon} and the \lean{VerifiedMRDT} package; keep the union model until
the runtime measurement is in.
