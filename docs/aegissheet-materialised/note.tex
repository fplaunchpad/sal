...
history. Nothing here is machine-checked; the mechanization target is a
product Join proof.
\end{abstract}
...
staged. Nothing in this note is machine-checked. The Python reference is
validated against the Lean model only on the twenty SPOT fixtures listed in
...
\midrule
H1, named removal & validated; conjectured in general & no counterexample in 3000 executions, 117{,}290 versions, 21{,}375 registered-ancestor and 851 virtual-base merges, three seeds; Lean proof owed \\
H1, clearing removal & validated for observation; refuted for replay & converges and matches the reference; 103 of 3000 executions have a merge that differs from timestamp-order replay \\
H2 & refuted & three hand-derived witnesses checked by the reference (Section~\ref{sec:h2}); 16 to 55 failing executions per 1000 in the harness \\
...
Undo revives a later removal & finding, reproduced & Section~\ref{sec:undo}; a property of the current model \\
\bottomrule
\end{tabular}
...
merge result with the timestamp-order replay of the union.
...
replay policy. The named variant makes every pair of operations commute,
which is the route every existing production instance uses.
...
keep registers, clearing removal & 27 & 31 & 45 & merge $\neq$ replay \\
drop unreferenced dead registers, named & 28 & 23 & 16 & position of revived axis \\
drop all dead registers, named & 10 & 10 & 7 & range resolution \\
ancestor ignored (control) & 186 & 173 & 146 & cells, ranges \\
eager range re-anchoring (control) & 634 & 642 & 683 & ranges \\
\bottomrule
\end{tabular}
\caption{Failing executions out of 1000 per seed. Each seed produced about
39{,}000 versions, 31{,}500 operations, 7{,}100 merges at a registered
ancestor, and 280 merges at a virtual base.}
\label{tab:campaign}
\end{table}
...
restricting undo to live axes (55 failing executions of 1000 at seed~7). The
...
register could be dropped at removal.
...
specification and should be decided before the Lean port.
...
Doubling the history quadruples the reference and doubles the materialised
state. The gap at 184 operations is a factor of 82. These are counts of
stored timestamps in the Python models, not bytes of any runtime.

\section{Controls}
\label{sec:controls}

\paragraph{Positive control.} The Python reference reproduces twenty
hand-derived expectations copied from the Lean SPOTs: update defeats
...
observed-remove sets and last-writer-wins registers. With named removal
every operation pair commutes, so the \lean{Join} proof can follow the
universal-equation route, or the product theorem \lean{joinAt\_prod} with a
per-component \lean{CanonicalJoinLaws} in the style of the multi-value
register. The sequential certificate needs a new representation relation;
\lean{no\_view\_only\_step} fixes that it must retain tokens, cell version
identities, and range version identities, which the materialised state does.

\section{Open obligations}

\begin{enumerate}
\item Decide the undo-revival question of Section~\ref{sec:undo} before
porting; it changes the sequential specification.
\item Model purge in the harness and test H3: purge as version deletion,
concurrent writes surviving, and the register-dropping rule at causal
stability.
...
current one; prove \lean{Join} and the sequential certificate; keep the
