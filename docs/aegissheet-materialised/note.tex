...
history. Purge needs a mask of cutoff and coordinates per marker, because the
current model deliberately masks writes concurrent with the marker and below
its cutoff, but it needs no acknowledgement protocol: dead identifiers can be
retired immediately once the register is kept. The witnesses are machine-checked as
...
H3, strict & refuted & purge as plain deletion diverges in 6, 3, 3 executions per 1000, always on a write concurrent with a marker and below its cutoff \\
H3, with mask & validated & a mask of cutoff and coordinates per marker reproduces the reference; 0 failing executions \\
...
\paragraph{Strict deletion is refuted.} A materialised purge that deletes
...
\paragraph{A mask suffices.} Keeping, per marker, the pair (cutoff,
coordinates) and filtering versions through the accumulated masks at apply
and at merge reproduces the reference with no failing execution. The mask is
the irreducible purge residue. The marker's covered entries are unnecessary
because tokens live in their own component, and its acknowledgements are
...
One decision is open, D2: whether a write concurrent with a purge and below
its cutoff should be masked, as the current model does, or kept, as the
maskless design does; the harness follows the current model.
...
Materialised state, update, merge, purge mask, retirement & none & specified; Python implementation \\
...
\item Decide D2 (Section~\ref{sec:h3}): mask or keep a write concurrent
with a purge and below its cutoff. The Lean port carries the mask either way
if the current model's semantics is kept.
