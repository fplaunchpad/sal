...
\paragraph{Tombstone RGA.}
RGA is staged because its event representation retains both insertion records
and deletion tombstones.  The intended collector needs stable-frontier
evidence authorizing removal of a dead insertion while retaining enough
anchor and order information for concurrent descendants and stale branches.
Formally, it must instantiate $C$, $W$, and $\mathsf{Rep}$ above and prove
update, ternary-merge, list-query, and continuation preservation.  Until those
obligations are discharged, its registry entry contains only the exact-state
baseline.
...
Only AegisSheet, TreeMove, and rich SidedPeritext currently package
representation-changing proofs.  The roadmap treats every remaining
