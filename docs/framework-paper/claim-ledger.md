...
| Client correctness uses one `rc` policy; `loOn` derives the sole order by retaining visibility for conflicting pairs and adding selected concurrent edges. | `Metatheory/Join/SetRelativeReplay.lean`: `loOn`; `Framework/Certificates.lean`: `SequentialSpec`; `Metatheory/Correctness.lean`: `IsSpecLinearizable`, `VerifiedMRDT` | Machine-checked public interface. |
| The LWW register's commuting `max` updates use the same timestamp-directed `rc` throughout replay laws, Join, and sequential correctness. Every acyclic policy is admitted for all-commuting updates by the sufficient replay laws; the old noncommutation biconditional is refuted. | `Instances/LWWRegister.lean`: `rc`, `replayLaws`, `join`, `rc_order_acyclic`, `timestamp_chain`, `canonical_respects`, `verified`, `chronological_winner`, `lower_timestamp_does_not_win`, `ordered_updates_commute`, `concrete_noncomm_iff_rc_refuted`; `Framework/ReplayLaws.lean`: `ReplayLaws.of_all_comm` | Machine-checked. The timestamp-max representation and ordering policy are trusted definitions; no JavaScript correspondence is claimed. |
| The retired tagged OR-set fixture refines to an ordinary `Finset` specification. Its `rc` leaves cross-key operations unordered and resolves same-key remove/add conflicts remove-first, yielding add-wins. | `Metatheory/Countermodels/TaggedORSet.lean`: `rc`, `spec`, `setSequentialCorrectness`, `cross_key_rc_either`, `same_key_rc_add_wins`, `observed_remove_removes` | Machine-checked. Observed tags remain implementation metadata, not sequential state. |
| Tombstone RGA refines to an ordinary `List Nat` specification. Its issuer accepts only root or live anchors, while tombstones remain internal for concurrent integration; its canonical witness respects the RGA `rc`. | `Instances/RGA.lean`: `applicable`; `Instances/RGASequential.lean`: `listSpec`, `rc`, `canonical_respects_rc`, `verified`; `Instances/RGAGC.lean`: `future_after_dead_anchor_not_applicable` | Machine-checked. |
...
The enriched FugueMax package is checked separately from the old coordinate-only
`FMSig`: `FugueMax.verified` supplies plain-list legality and refinement, and
`FugueMax.correct` supplies maximal non-interleaving for the same witness and
certified execution. The exact issuer stores immutable birth/origin metadata
implementation-side. Evidence: `Instances/FugueMaxContract.lean`,
`Instances/FugueMaxImplementation.lean`, and the exact production gate in
`public-contracts.json`. This is a mechanized model-level claim; datatype-state
collection and an independent FugueMax runtime are not supplied.
