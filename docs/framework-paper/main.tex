...
each released datatype and labels representation experiments separately. We
call Lean declarations \emph{machine-checked}, runtime test results
\emph{validated}, and benchmark observations \emph{measured}.
...
SidedEmbedRGA & \checkmark & \checkmark & \checkmark & policy lemmas \\
...
The separate negative ledger checks the MVR single-register counterexample,
the queue FIFO counterexample, and internal policy models. In particular,
FugueMax's coordinate-level \lean{FMSig} has issuance, convergence, and
RA-linearizability theorems, but it is not a public datatype package. Its role
is to justify ordering-policy properties used by the released
\lean{ProductionRGA.sided} package. This distinction prevents a partial proof
from appearing in the production table.

The production ledger also checks product construction, Peritext rendering,
the distributed collector, and the combined GC theorem. The cited instance
files print standard Lean axioms only; the repository release script rejects
\lean{sorry} and \lean{sorryAx} in the production tree and validates the
