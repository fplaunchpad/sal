...
reachability-conditioned), so it forces the conditioned route, where the
last obstruction is now pinned to a single lemma: the peel bubble cannot
certify applicability at the \emph{interleaved} states its $\sigma$-walk
visits. Whether that side-condition holds for the RGA --- closing the
hosting --- or forces a reachability-restricted bubble is the final open
step.
\end{openq}
