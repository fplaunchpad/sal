...
\date{July 3, 2026 (revised July 7, 2026)}
...
end-to-end RA-linearizability theorems through it. The one data type
...
reachable configuration, from a single honest-delivery assumption.
\end{abstract}
...
(the closure-indexed investigation lives under \lean{Development/}, each
cited theorem kernel-checked).
...
\bottomrule
\end{tabular}}
\caption{The Sal production catalogue. Nine of twelve carry
kernel-checked end-to-end RA-linearizability through the eight VCs, all
discharges in a single \lean{MRDT\_Instances.lean}; the tombstone-free
RGA --- provably outside the VC set --- carries a kernel-checked
\emph{conditioned} end-to-end theorem (RA-linearizability up to
observational equivalence) through the extension of \S\ref{sec:open}.}
\label{tab:catalog}
\end{table}
...
\begin{openq}[the feasible update layer --- resolved: from refuted
conditionings to a conditioned metatheory]
...
\lean{loOnA\_noopFeasible\_verdict}). The general convergence theorem then
factors cleanly (\lean{ConditionedConvergence}): a \emph{single} order VC
(``a vacuous $\mathsf{commutesOn}$ edge is an applicability dependency'')
re-adds exactly the dropped edges, reducing conditioned convergence to the
unconditioned theorem (\lean{conditioned\_convergence\_on}) --- for any
data type whose commutation is available unconditionally. The
tombstone-free RGA is precisely the one that is not (its commutation is
reachability-conditioned), so it forces the conditioned route. There the
last obstruction resolves sharply (\lean{RGA\_Rehoming\_Gate}): the
interleaved-applicability side-condition is \emph{false} for the RGA --- a
concurrent delete rehomes nodes and stales a concurrent insert's path, and
the staled insert is neither applicable nor a no-op --- \emph{yet the
version still converges}, because the insert re-anchors at exactly the
node the delete rehomed to (the RGA's own \lean{insdel\_comm}). So the
obstruction is not unreachability, and not non-convergence: it is that the
peel bubble demands \emph{pointwise} applicability at hybrid fold states,
while a path-carrying data type supplies \emph{semantic} commutation only
at the pair's common causal base. That architecture now exists, and it is not a repaired bubble: the swap
frame itself is refuted one level higher. At a merge union, even
\emph{no-op-feasible} witnesses are unsatisfiable --- an LCA-first
enumeration pre-applies one branch's anchor-killing deletes before
replaying the other branch's inserts, staling their recorded paths
(\lean{RGA\_HEnum\_Refutation}) --- and on a seven-event two-branch
execution \emph{no} born-applicable enumeration of the union replays
both branches' recorded observations at all: tombstone-free rehoming
makes the \emph{order of concurrent deletes} observable in survivors'
stored parents, and the two branches observe it differently. The raw
$\mathsf{do}$-fold is the only sequentially replayable semantics of a
merge union, so the \emph{target} must move: RA-linearizability up to
the data type's observational equivalence $\approx$.

The resolution is a conditioned \emph{metatheory} in four moves, each
kernel-checked. (i)~An observational quotient $D \mapsto D_\approx$:
states are $\approx$-classes of invariant states; update, merge and the
applicability guard descend by congruence. (ii)~A canonical-witness
discipline: each version carries, existentially, an enumeration of its
events satisfying the RGA engine's own per-step discipline --- fresh
nonzero ids, no id reuse, live recorded chains --- plus payload honesty;
the discipline \emph{extends} at applies (freshness is a field of the
transition itself) and \emph{joins} at merges, the union witness being
the LCA-then-delta concatenation --- no reordering, no swaps anywhere.
(iii)~A generation discipline, derived rather than assumed: every event
is accurate at the fold of its \emph{transitive dependencies}, provable
from \emph{born accuracy} --- each op was generated against a causal
fold of the events its replica had seen, which is simply how a client
computes. (iv)~Per-survivor \emph{birth-anchor bridges} reduce merge
canonicity to record coherence at dependency folds, where the
generation discipline makes every recorded chain live. The end-to-end
theorem (\lean{rga\_tombstone\_free\_ra\_linearizable3\_eq}, mainline in
\lean{RGA\_TombstoneFree\_RA\_Lin.lean}): every reachable configuration
of the quotient store is RA-linearizable up to $\approx$, per version,
from a single honest-delivery assumption --- born accuracy plus
applicable delivery. Everything else is structural or derived: Lamport
clocks and timestamp uniqueness are fields of the configuration
(dishonest-clock executions are unrepresentable), and nonzero ids and
nonzero delete targets follow from the delivered op's own
wellformedness. What this question asked for --- a proof architecture
that can see why the RGA converges --- exists; the price, stated
honestly, is the move from equality to $\approx$ in the linearization
target and the born-accuracy clause in the trust statement, both
forced: the first by rehoming's observable delete order, the second by
tombstone-freedom itself.
\end{openq}
...
conditioned convergence (order-half) & \lean{conditioned\_convergence\_on} & \lean{Development/ConditionedConvergence.lean} \\
RGA rehoming: oracle false, converges & \lean{verdict\_oracle\_false\_but\_converges} & \lean{Development/RGA\_Rehoming\_Gate.lean} \\
...
observational quotient $D \mapsto D_\approx$ & \lean{QSig}, \lean{qapplicable} & \lean{Development/GenericEqQuotient$\ast$.lean} \\
canonical-witness layer, raw-$\approx$ target & \lean{IsRALinearizable3Eq}, \lean{RA\_linearizable\_up\_to\_eq\_H} & \lean{Development/GoodConfig3H.lean} \\
RGA canonical engine & \lean{CanonInv}, \lean{canon\_fold} & \lean{Development/RGA\_Canon$\ast$.lean} \\
generation discipline from born accuracy & \lean{genDisc2C\_of\_born} & \lean{Development/RGA\_GenDisc\_Assembly.lean} \\
capstone + discharged leaves & \lean{rga\_RA\_linearizable\_skeleton3} & \lean{Development/RGA\_Skeleton3.lean}, \lean{RGA\_$\ast$\_Discharge.lean} \\
honest residual & \lean{HonestDelivery}, \lean{rga\_RA\_linearizable\_honest} & \lean{Development/RGA\_Honest\_Residual.lean} \\
mainline entry point & \lean{rga\_tombstone\_free\_ra\_linearizable3\_eq} & \lean{RGA\_TombstoneFree\_RA\_Lin.lean} \\
\bottomrule
\end{tabular}}
\end{center}
...
The conditioned-metatheory artifacts under \lean{Development/} are the
research record --- they establish the refutations and the
closure-indexed resolution cited in \S\ref{sec:defeater} and the open
questions, and carry the full conditioned chain culminating in the
mainline tombstone-free-RGA theorem; each cited theorem is kernel-checked
