...
states $\sigma(E)$, and a ternary \emph{Join Lemma} whose induction
...
The update side of the theory never mentions the merge: over
$\langle \Sigma, \Sinit, \mathsf{do}, \mathsf{rc}\rangle$ alone, the
update-layer VCs (items 1--3 of \S\ref{sec:vcs}) yield the
\textbf{convergence theorem} --- any two $\loE{E}$-respecting enumerations
of a finite event set $E$ fold from $\Sinit$ to the same state. (This is
where the set-relative reading of \S\ref{sec:setting} earns its keep:
relative to the paper's configuration-wide order, convergence over a
version's own event set is \emph{false} --- the defeater's heads are
already witnesses.) Convergence makes every event set denote a state, the
\textbf{canonical state} $\sigma(E)$, and witness lists disappear from
the induction: a version linearizes iff it holds the canonical state of
its event set (respecting enumerations exist by acyclicity of $\loE{E}$,
from VC~2). The one genuinely
...
For state-based CRDTs --- binary merge, no LCA --- the natural
merge-coherence laws are lattice-flavoured: associativity, idempotence,
inflation of updates. Each of the three provably fails under ternary
merge.
