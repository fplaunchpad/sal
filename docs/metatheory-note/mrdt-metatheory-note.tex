...
declares its closure strength $\mathcal{C}\in\{\text{weak},\text{full}\}$,
with a single soundness bridge (store versions are fully causally closed,
hence $\mathcal{C}$-closed for either strength). The two routes of
\S\ref{sec:adequacy} are the two instances; the disjunction is the
contract, not a defect to be reunified. Genuinely open: whether the
single-strength contract (carrying the common witness set as data,
re-founding $\cd$ on the full downset) buys anything over the
closure-indexed form.
\end{openq}
...
applicability \emph{depends} on the other), or --- strictly more general
(\lean{G2\_Applicability\_Aware}: \lean{separating\_inequivalence}, a
negative dependency between concurrent deletes that no positive
create-before-use relation captures) --- convergence must range only over
\emph{applicability-valid} enumerations. A last twist keeps this open:
genuinely reachable versions can carry \emph{redundant concurrent} ops
(two concurrent deletes of one node) for which \emph{no} enumeration is
applicability-valid at every prefix, so the right feasibility notion must
tolerate \emph{applicable-or-no-op} re-application.
\end{openq}
