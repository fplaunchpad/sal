...
import CaseStudies.Neem_interfaces.Map_extended_with_lean_set
...
@[simp]
def mysel (s: concrete_st) (k: ℕ) [Decidable (contains s k)]: (set ℕ) :=
if (contains s k) then (sel s k) else {}
