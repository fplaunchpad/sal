...
State is a single ℕ; the only operation is `Write v`, which sets the
state to `min state v`. Merge is pointwise min. With `init_st = 0` this
CRDT is semantically degenerate on ℕ (the state stays 0 forever), but
the RA-linearizability VCs are lattice facts about `min`, so all 24
