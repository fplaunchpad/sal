...
/-!
# Insert-Only Priority Queue CRDT

State is a `map (priority × replica_id × timestamp) element_id`. A
`Push prio elem` at operation `(ts, rid, _)` writes the element id at
the key `(prio, rid, ts)`. Under the paper's `distinct_ops` assumption
(unique ts per operation), each key is globally unique, so two
distinct operations modify disjoint keys and commute, and merge
reduces to the same pointwise-max pattern as the grow-only counter.

This is the insert-only (grow-only) version of a priority queue: there
is no state-based Pop. A client wanting the highest-priority element
reads the state and takes the minimum key prefix. State-based Pop
...
match o with
| (ts, (rid, .Push prio elem)) =>
    upd s (prio, rid, ts) (max (mysel s (prio, rid, ts)) elem)
...
def merge (a b : concrete_st) : concrete_st :=
  ...
  iter_upd (fun k _ => max (mysel a k) (mysel b k)) u
