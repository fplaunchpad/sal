...
State is a `map ℕ (ℕ × ℕ)` — element ID → (add_ts, remove_ts).
Operations `Add id` and `Remove id` update the add- and remove-timestamps
respectively by taking a max with the incoming timestamp. Merge is the
pointwise max of both timestamps over the union of the domains. All
operations commute.
-/

@[simp] abbrev concrete_st := map ℕ (ℕ × ℕ)
/- keys: element IDs, values: (add_ts, remove_ts) -/
...
match o with
| (ts, (_, .Add id)) => upd s id (max (mysel s id).1 ts, (mysel s id).2)
| (ts, (_, .Remove id)) => upd s id ((mysel s id).1, max (mysel s id).2 ts)
...
let u := const_on keys (0, 0)
iter_upd (fun k _ => (max (mysel a k).1 (mysel b k).1, max (mysel a k).2 (mysel b k).2)) u
...
→
(rc o1 o2 = rc_res.Either ↔ commutes_with o1 o2) := by sorry -- TODO: sal stage 3 aesop norm-simp explodes on nested pair/map unfolds
...
:= by sorry -- TODO: sal stage 3 aesop norm-simp explodes on nested pair/map unfolds
...
→
eq (do_ (do_ (do_ s o1) o2) o3) (do_ (do_ (do_ s o2) o1) o3) := by sorry -- TODO: sal stage 3 aesop norm-simp explodes on nested pair/map unfolds



theorem merge_comm (a b: concrete_st) :
eq (merge a b) (merge b a) := by sorry -- TODO: sal stage 3 aesop norm-simp explodes on nested pair/map unfolds


theorem merge_idem (s: concrete_st) :
eq (merge s s) s := by sorry -- TODO: sal stage 3 aesop norm-simp explodes on nested pair/map unfolds
...
eq (merge (do_ a ol) (do_ b ol)) (do_ (merge a b) ol) := by sorry -- TODO: sal stage 3 aesop norm-simp explodes on nested pair/map unfolds
