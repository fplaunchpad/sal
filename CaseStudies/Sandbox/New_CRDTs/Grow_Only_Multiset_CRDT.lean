...
/-!
# Grow-Only Multiset (Bag) CRDT

State is a map from `(replica_id, element_id)` pairs to `Int`, tracking the
number of times each replica has added each element. An element's
multiplicity in the multiset is the sum over all replicas of the count for
that `(rid, eid)` pair. Merge takes the per-key maximum so counts are never
lost.
-/
...
match o with
| (_, (rid, app_op_t.Add eid)) => upd s (rid, eid) (mysel s (rid, eid) + 1)
