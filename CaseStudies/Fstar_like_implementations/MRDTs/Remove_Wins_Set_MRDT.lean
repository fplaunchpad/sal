...
@[simp]
def init_st : concrete_st:= add (0,1) (add (0,2) empty)
...
match o with
| (_, (rid, .Add1)) => filter s (fun e => Prod.snd e != 1)
| (ts, (rid, .Rem1)) => add (ts,1) s
| (_, (rid, .Add2)) => filter s (fun e => Prod.snd e != 2)
| (ts, (rid, .Rem2)) => add (ts,2) s
...
→
(rc o1 o2 = rc_res.Either ↔ commutes_with o1 o2) := by sorry -- TODO: Blaster removed in v4.28 bump
