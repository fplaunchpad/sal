...
theorem ind_left_2op (l : concrete_st) (a: concrete_st) (b: concrete_st) (o1: op_t) (o2: op_t) (o1': op_t) :
 ...
 := by
 -- Since o1 and o2 are non-commuting, the rc results for o1 and o2 must be either Fst_then_snd or Either. We can split into these two cases.
   intro h
   cases' h with h opc h distinct_o1 h distinct_o2 h comm;
   -- Since o1' is distinct from both o1 and o2, the subsequent operations on o1' and o1 don't affect each other. Therefore, the merge operation should be associative in this case.
   have h_assoc : ∀ s : concrete_st, eq (merge l (do_ (do_ s o1') o1) (do_ b o2)) (do_ (merge l (do_ s o1') (do_ b o2)) o1) := by
     intro s
     simp [merge, do_, eq];
     intro id; by_cases h : id = o1.2.2.1 <;> simp +decide [ h ] ;
     -- Since sets are unordered, the union of multiple sets is equal to the union of their individual unions.
     ext x; simp [union];
     by_cases h : l.domain o1.2.2.1 = Bool.true <;> by_cases h' : s.domain o1.2.2.1 = Bool.true ∨ o1.2.2.1 = o1'.2.2.1 <;> by_cases h'' : b.domain o1.2.2.1 = Bool.true ∨ o1.2.2.1 = o2.2.2.1 <;> simp +decide [ h, h', h'' ];
     · grind;
     · grind;
     · grind +ring;
     · exact?;
...
:= by
  -- By definition of `do_`, we know that `do_ a o1'` and `do_ b ol` are equal to `a` and `b` respectively.
  simp [do_] at *;
  -- By definition of commutativity, we need to show that the order of applying the operations does not affect the final result.
  intros h_comm;
  -- Since the keys are distinct, the order of applying the operations does not affect the final result.
  intros h1 h2 h3
  simp [h1, h2, h3] at *;
  -- By simplifying the if-then-else expressions, we can see that they are equivalent due to the commutativity of the union operation and the fact that the keys are distinct.
  intros id
  by_cases h_id1 : id = o1.2.2.1
  by_cases h_id2 : id = o1'.2.2.1
  by_cases h_id3 : id = ol.2.2.1;
  · simp_all +decide [ Prod.ext_iff ];
    ext x; simp +decide [ union ] ;
    grind;
  · -- Since the keys are distinct, the if-then-else expressions simplify to the same value.
    simp [h_id1, h_id2, h_id3] at *;
    -- Since the keys are distinct, the disjunctions in the if-then-else expressions will evaluate to true.
    simp [h_id2, h_id3] at *;
    ext; simp [union];
    grind +ring;
  · by_cases h_id3 : id = ol.2.2.1 <;> simp +decide [ h_id1, h_id2, h_id3 ] ;
    · by_cases h : l.domain o1.2.2.1 = Bool.true ∨ o1.2.2.1 = ol.2.2.1 <;> simp +decide [ h ];
      · ext; simp +decide [ union ] ;
        grind +ring;
      · grind +ring;
    · by_cases h : l.domain o1.2.2.1 <;> by_cases h' : a.domain o1.2.2.1 <;> by_cases h'' : b.domain o1.2.2.1 <;> simp +decide [ h, h', h'' ];
      all_goals simp_all +decide [ funext_iff, union ] ;
      · grind +ring;
      · grind +ring;
      · grind +ring;
      · grind +ring;
