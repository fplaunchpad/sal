...
@[simp]
instance : Ord (ℕ × ℕ) where
  compare := fun (a1, a2) (b1, b2) =>
    match compare a1 b1 with
    | .lt => .lt
    | .gt => .gt
    | .eq => compare a2 b2

instance : Std.TransCmp (compare : ℕ × ℕ → ℕ × ℕ → Ordering) where
  eq_swap :=
  by sorry
  isLE_trans := by sal


-- Bounded Counter MRDT state: (counter_value, transfer_map)
-- transfer_map tracks quota transfers between replicas: (sender, receiver) -> amount

abbrev concrete_st := Int × Std.ExtTreeMap (ℕ × ℕ) Int
...
-- Helper functions to access state components
@[simp]
def get_value (s: concrete_st) : Int := Prod.fst s

@[simp]
def get_transfers (s: concrete_st) := Prod.snd s

-- Calculate quota for a replica (simplified: just the counter value for now)
@[simp]
def quota (_replica: ℕ) (s: concrete_st) : Int :=
(Std.ExtTreeMap.foldl (fun value (sender, receiver) transferred =>
if sender = _replica then value - transferred
else if receiver = _replica then value + transferred
else value) (get_value s) (Prod.snd s))
...
@[simp]
def inc (s: concrete_st) : concrete_st :=
  (get_value s + 1, get_transfers s)

-- Decrement operation - may fail if insufficient quota (decrement by 1)
@[simp]
def dec (replica: ℕ) (s: concrete_st) : concrete_st :=
  let q := quota replica s
  if q >= 1 then
    (get_value s - 1, get_transfers s)
  else
    s  -- If insufficient quota, state remains unchanged

-- Transfer quota from sender to receiver
@[simp]
def transfer (sender: ℕ) (receiver: ℕ) (amount: Int) (s: concrete_st) : concrete_st :=
  let q := quota sender s
  if q >= amount then
    let transfers := get_transfers s
    let pair := (sender, receiver)
    let value := if transfers.contains pair then transfers.get! pair else 0
    let new_transfers := transfers.insert pair value
    (get_value s, new_transfers)
  else
    s  -- If insufficient quota, state remains unchanged
...
match o with
| (_, (_replica, app_op_t.Inc)) => inc s
| (_, (replica, app_op_t.Dec)) => dec replica s
| (_, (sender, app_op_t.Transfer receiver amount)) => transfer sender receiver amount s
...
-- Three-way merge for MRDT: merge(lca, left, right)
@[simp]
def merge (l a b: concrete_st) : concrete_st :=
  -- Merge counter value using standard 3-way merge
  let merged_value := get_value a + get_value b - get_value l

  -- Merge transfer maps (take max for each key)
  let transfers_a := get_transfers a
  let transfers_b := get_transfers b
  let merged_transfers := Std.ExtTreeMap.mergeWith (fun _ v1 v2 => max v1 v2) transfers_a transfers_b

  (merged_value, merged_transfers)

@[simp]
def commutes_with (o1 o2: op_t) :=
    forall s, eq (do_ (do_ s o1) o2) (do_ (do_ s o2) o1)

-- theorem merge_map_max : ∀ (m1 m2 : Std.ExtTreeMap (ℕ × ℕ) ℤ),
-- Std.ExtTreeMap.mergeWith (fun _ v1 v2 => max v1 v2) m1 m2


-- Test cases for increment and decrement
-- op_t = (op_id, replica_id, operation)

-- Test 1: Increment from initial state
-- Initial state: (0, {})
-- After Inc: (1, {})
#eval! do_ init_st (0, 0, app_op_t.Inc)  -- Expected: (1, {})

-- Test 2: Two increments
#eval! do_ (do_ init_st (0, 0, app_op_t.Inc)) (1, 0, app_op_t.Inc)  -- Expected: (2, {})

-- Test 3: Increment then decrement (should succeed since quota = 1)
#eval! do_ (do_ init_st (0, 0, app_op_t.Inc)) (1, 0, app_op_t.Dec)  -- Expected: (0, {})

-- Test 4: Decrement from initial state (should fail - no quota)
#eval! do_ init_st (0, 0, app_op_t.Dec)  -- Expected: (0, {}) - unchanged

-- Test 5: Two increments then two decrements
#eval! do_ (do_ (do_ (do_ init_st
  (0, 0, app_op_t.Inc))   -- (1, {})
  (1, 0, app_op_t.Inc))   -- (2, {})
  (2, 0, app_op_t.Dec))   -- (1, {})
  (3, 0, app_op_t.Dec)    -- Expected: (0, {})

-- Transfer test cases
-- Transfer (receiver, amount) - sender is the replica_id in op_t

-- Test 6: Transfer with no quota (should fail)
-- Replica 0 tries to transfer 1 to replica 1, but has no quota
#eval! do_ init_st (0, 0, app_op_t.Transfer 1 1)  -- Expected: (0, {}) - unchanged

-- Test 7: Increment then transfer
-- Replica 0 increments (gets quota 1), then transfers 1 to replica 1
#eval! do_ (do_ init_st
  (0, 0, app_op_t.Inc))           -- (1, {}) - replica 0 has quota 1
  (1, 0, app_op_t.Transfer 1 1)   -- Should record transfer from 0 to 1

-- Test 8: Transfer then decrement by receiver
-- Replica 0 increments, transfers to replica 1, then replica 1 decrements
#eval! do_ (do_ (do_ init_st
  (0, 0, app_op_t.Inc))           -- (1, {})
  (1, 0, app_op_t.Transfer 1 1))  -- Transfer 1 from replica 0 to replica 1
  (2, 1, app_op_t.Dec)            -- Replica 1 decrements (should succeed with transferred quota)

-- Test 9: Multiple replicas scenario
-- Replica 0 increments twice, transfers 1 to replica 1, both decrement
#eval! do_ (do_ (do_ (do_ (do_ init_st
  (0, 0, app_op_t.Inc))           -- (1, {})
  (1, 0, app_op_t.Inc))           -- (2, {})
  (2, 0, app_op_t.Transfer 1 1))  -- Transfer 1 from 0 to 1
  (3, 0, app_op_t.Dec))           -- Replica 0 decrements (has quota 2-1=1)
  (4, 1, app_op_t.Dec)            -- Replica 1 decrements (has transferred quota 1)
...
unfold Std.ExtTreeMap.mergeWith
