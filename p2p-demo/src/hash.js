// Pure-JS SHA-256 (browser + Node, no deps). Used to CONTENT-ADDRESS commits.
//
// #95 asks the demo to swap the runtime sync layer's model FNV hash
// (src/sync.js) for a real SHA. We do that here: p2p-demo/src/node.js names
// every commit by the SHA-256 of its canonical content (a Merkle DAG, git
// style: a commit's id folds in its parents' ids), so the SAME id names the
// SAME commit on every peer AND on disk (git persistence). One hash for the
// wire AND the store, closing the FNV collision gap the runtime README flags.
//
// This is a standard FIPS-180-4 SHA-256 over a UTF-8 string. It is checked
// bit-for-bit against node:crypto in test/node.test.js (the empty string and
// "abc" NIST vectors plus randomized agreement), so the demo does not rest on
// a hand-rolled hash being merely plausible.
