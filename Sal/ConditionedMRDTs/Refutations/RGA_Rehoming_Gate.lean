...
/-!
# Does `interleavingFeasible` hold for the tombstone-free RGA?

`ConditionedConvergence.lean` closes the *order* half of conditioned
convergence and localizes the whole residual RGA obstruction to a single
oracle: the conditioned bubble
...
2. **The staled `Ins` is NOT a no-op** (`staled_ins_not_noop`): at
   ...
   `appOrNoop` fails, and therefore

   > **`interleavingFeasible RGACondSig` is FALSE** (`not_interleavingFeasible_RGA`).

3. **But the failure is the ORACLE's, not the RGA's.**
   `interleavingFeasible` is *sufficient* for the bubble, but it is
...
The obstruction is NOT unreachability: the
...
namespace Sal.ConditionedMRDTs.RGARehomingGate

open Sal.Emulation
open Sal.ConditionedMRDTs.RGASig (RGACondSig)
open Sal.ConditionedMRDTs.G2Probe (contains_doIns_self)
open Sal.ConditionedMRDTs.ConditionedConvergence (interleavingFeasible appOrNoop)
...
This makes `interleavingFeasible` FALSE for the RGA. -/
...
end Sal.ConditionedMRDTs.RGARehomingGate
