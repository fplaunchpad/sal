...
namespace Sal.ConditionedMRDTs.UpdateFeasibilityGate

open Sal.Emulation
open Sal.ConditionedMRDTs.G2Probe
...
`ConditionedConvergence.appliesDependsOn RGACondSig delOpE insOpE` holds: `delOpE` is inapplicable at
...
theorem rga_appliesDependsOn_del_ins : ConditionedConvergence.appliesDependsOn RGACondSig delOpE insOpE := by
  refine ⟨init_st, fun h => ?_⟩
  have hnotP : ¬ RGACondSig.applicable delOpE init_st :=
    Sal.ConditionedMRDTs.UpdateFeasibilityGate.delNode1_not_applicable 2 0 init_st (by simp [init_st])
  have hQ : RGACondSig.applicable delOpE (do_ init_st insOpE) :=
    Sal.ConditionedMRDTs.G2Probe.delOpE_applicable_after_ins
  exact hnotP (cast h.symm hQ)

#print axioms rga_appliesDependsOn_del_ins

end Sal.ConditionedMRDTs.UpdateFeasibilityGate
