...
namespace Sal.ConditionedMRDTs.RGASimulInduction2

open Sal.Emulation
open Sal.ConditionedMRDTs.RGAGeneralSwap (Faithful ClimbFaithful DelTargetFaithful NoFreshClash)
...
open Sal.ConditionedMRDTs.RGASig (RGACondSig isAncPath_not_mem)
open RGAInterleavedThreading
  (GoodStep GoodFold AncInsLink chainFaithful_goodFold chainFaithful_init_recList)
open Sal.ConditionedMRDTs.RGAGenDischarge
  (NonDegen ReachInv noFreshClash_of_accurate_fresh freshId_not_mem_recList)
open Sal.ConditionedMRDTs.RGAGenDischarge2 (IsDepPre GenDisc2)
open Sal.ConditionedMRDTs.RGAUpdateConvergenceFinal
  (fresh_ts_config goodFold_of_stepwise goodStep_ins_concurrent)
open Sal.ConditionedMRDTs.RGASimulInduction
...
theorem conv_step2 (C : ConditionedConfiguration RGACondSig)
    ...
    have hπ₂ : π₂ = [] := by
      ...
      | cons a l =>
        exact absurd ((hm₁ a).mpr ((hm₂ a).mp (by rw [hπ]; exact List.mem_cons_self)))
          List.not_mem_nil
    rw [hπ₂]
    exact Sal.ConditionedMRDTs.RGAConditionedConvergence.eq_refl _
  · rw [List.concat_eq_append] at hcat
    ...
    rw [e₁, e₄]
    refine Sal.ConditionedMRDTs.RGAConditionedConvergence.eq_trans _ _ _ hstep ?_
    rw [← e₂, e₃]
    exact hbubble
...
end Sal.ConditionedMRDTs.RGASimulInduction2
