...
The composite `PeritextTF := RGA_TF ⊗ MarkStore` proves convergence and safety
(`peritextTF_ra_linearizable_up_to_eq`) under a premise
...
3. **Decorative for linearizability, load-bearing for the read.** The
   ...
   (`peritextTF_ra_linearizable_honest`) — because the linearizability proof
...
theorem peritextTF_ra_linearizable_honest
  ...
  peritextTF_ra_linearizable_up_to_eq (honestDeliveryPlus_imp hHD) C hReach
...
#print axioms peritextTF_ra_linearizable_honest
