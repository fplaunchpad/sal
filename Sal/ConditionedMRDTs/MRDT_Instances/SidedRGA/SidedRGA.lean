...
below is policy-free: `Side` is one more field the effect stores blindly.

Coordinates here are **symbol lists** (`List ℕ`, the sided alphabet with
R band {1,2} < marker 3 < L band {4,5}), not bit strings: `sBlock`
already composes the side bit and the per-band (complemented) code, so
one `OrderedPrefixCode` parameter still covers both bands.

Sections mirror the one-sided instance file: §1 datatype · §1½ first
list algebra · §2 sortedness + canonical-form extensionality · §2½ the
sorted 2-merge · §3 well-formed enumerations and fold-canonicity ·
§5 honest histories · §6 the Join (the merge is its own linearization
...
def S (Γ : OrderedPrefixCode) : ConditionedMRDTSig where
  State := SState
...
structure SHonestCore (Γ : OrderedPrefixCode)
    (C : Sal.Emulation.Configuration (S Γ).toCRDTSig) : Prop where
...
theorem s_del_ins_mem {Γ : OrderedPrefixCode}
    {C : Sal.Emulation.Configuration (S Γ).toCRDTSig}
...
theorem s_goodConfig3 {Γ : OrderedPrefixCode} {C : Configuration (S Γ)}
    (hReach : SReach Γ C) : GoodConfig3 C :=
  goodConfig3_of_honest_reach (fun _ hHon => s_join_at (sHonest_core hHon))
    hReach
...
code Γ. -/
theorem sided_embed_ra_linearizable3 {Γ : OrderedPrefixCode}
    {C : Configuration (S Γ)} (hReach : SReach Γ C) :
    IsRALinearizable3 C :=
  isRALinearizable3_of_good (s_goodConfig3 hReach)

#print axioms sided_embed_ra_linearizable3
...
`P := sApplicable`: `GenHonest` + causal-past enumerability supply exactly
`sHonest_of_applicable`'s hypothesis. -/
theorem sHonest_of_genHonest {Γ : OrderedPrefixCode} (C : Configuration (S Γ))
...
contract becomes sublist preservation, exactly as one-sided: a delete never
reorders survivors; an insert never reorders the existing document; a merge
displays each branch's survivors in that branch's own order. -/

/-- **Delete-order preservation**: deletion displays exactly the survivors,
in unchanged order. -/
theorem sUpdate_del_sublist (Γ : OrderedPrefixCode) (s : SState)
    (ts r x : ℕ) :
    List.Sublist (sUpdate Γ s (ts, r, .del x)) s :=
  List.filter_sublist

theorem sInsert_sublist (r : SRec) : ∀ (s : SState), List.Sublist s (sInsert r s)
  | [] => List.nil_sublist _
  | x :: xs => by
      by_cases h : keyLt (sKey x.2.2) (sKey r.2.2) = true
      · rw [show sInsert r (x :: xs) = r :: x :: xs from by simp [sInsert, h]]
        exact List.Sublist.cons r (List.Sublist.refl _)
      · rw [show sInsert r (x :: xs) = x :: sInsert r xs from by
          simp [sInsert, h]]
        exact List.Sublist.cons₂ x (sInsert_sublist r xs)

/-- **Step stability**: an insert never reorders the existing document. -/
theorem sUpdate_ins_sublist (Γ : OrderedPrefixCode) (s : SState)
    (ts r : ℕ) (el : ℕ) (π : List ℕ) (a : ℕ) (sd : Side) :
    List.Sublist s (sUpdate Γ s (ts, r, .ins el π a sd)) := by
  simp only [sUpdate]
  by_cases h : ts ∈ sIds s
  · rw [if_pos h]
  · rw [if_neg h]
    exact sInsert_sublist _ s

theorem sMerge2_sublist_left : ∀ (as bs : SState), List.Sublist as (sMerge2 as bs) := by
  intro as bs
  induction as, bs using sMerge2.induct with
...
theorem sMerge2_sublist_right : ∀ (as bs : SState), List.Sublist bs (sMerge2 as bs) := by
  intro as bs
  induction as, bs using sMerge2.induct with
  | case1 ys =>
      rw [show sMerge2 [] ys = ys from by rw [sMerge2]]
  | case2 xs => cases xs <;> simp [sMerge2]
  | case3 a as' b bs' h ih =>
      rw [show sMerge2 (a :: as') (b :: bs') = a :: sMerge2 as' (b :: bs')
        from by rw [sMerge2]; simp [h]]
...
theorem sMergeL_stable_left (l a b : SState) :
    List.Sublist (a.filter (fun r => decide (r.1 ∈ sIds b ∨ r.1 ∉ sIds l)))
      (sMergeL l a b) :=
  sMerge2_sublist_left _ _

theorem sMergeL_stable_right (l a b : SState) :
    List.Sublist (b.filter (fun r => decide (r.1 ∉ sIds l ∧ r.1 ∉ sIds a)))
      (sMergeL l a b) :=
  sMerge2_sublist_right _ _

#print axioms sUpdate_del_sublist
#print axioms sMergeL_stable_left
