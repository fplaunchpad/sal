...
theorem l19_spec_replay :
    sSpecFold opsL19 = [(0, 0), (50, 50), (30, 30), (10, 10), (61, 61),
      (41, 41), (21, 21), (1, 1)] := by native_decide

/-- PASS: the rooted fold projects to the same buffer, on the same
trace. -/
theorem l19_fold_eq_spec :
    (sRFold unaryCode opsL19).map sProj = sSpecFold opsL19 := by
  native_decide

/-- FAIL pin: the interleaved order is NOT what the buffer program
produces. -/
theorem l19_spec_not_interleaved :
    sSpecFold opsL19 ≠ [(0, 0), (61, 61), (50, 50), (41, 41), (30, 30),
      (21, 21), (10, 10), (1, 1)] := by native_decide

/-- A genuinely sequential (monotone-stamp, applicable-at-every-fold)
two-sided trace: backward-type 10, 30, 50 to the left of 1,
forward-type 61, 70 to its right, then delete 30. -/
def ch61 : SChain := ch1 ++ [(Side.R, 60)]

def opsTwoSided : List (Op SOp) :=
  [ (1, 0, .ins 1 [] 0 Side.R)
  , (10, 0, .ins 10 (cOf ch1) 1 Side.L)
  , (30, 0, .ins 30 (cOf ch10) 10 Side.L)
  , (50, 0, .ins 50 (cOf ch30) 30 Side.L)
  , (61, 0, .ins 61 (cOf ch1) 1 Side.R)
  , (70, 0, .ins 70 (cOf ch61) 61 Side.R)
  , (80, 0, .del 30) ]

/-- PASS: fold = spec, hand-derived on both sides:
`[0, 50, 10, 1, 61, 70]`. -/
theorem two_sided_fold_eq_spec :
    (sRFold unaryCode opsTwoSided).map sProj = sSpecFold opsTwoSided := by
  native_decide

/-- PASS: the shared value, pinned. -/
theorem two_sided_display :
    sSpecFold opsTwoSided = [(0, 0), (50, 50), (10, 10), (1, 1),
      (61, 61), (70, 70)] := by native_decide

/-- FAIL pin: dropping the sentinel breaks the buffer equation shape. -/
theorem two_sided_not_unrooted :
    sSpecFold opsTwoSided ≠ [(50, 50), (10, 10), (1, 1),
      (61, 61), (70, 70)] := by native_decide

end SidedSeqSPOT

#print axioms SidedSeqSPOT.two_sided_fold_eq_spec
