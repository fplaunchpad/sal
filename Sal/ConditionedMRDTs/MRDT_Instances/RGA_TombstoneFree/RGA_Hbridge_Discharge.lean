import Sal.ConditionedMRDTs.MRDT_Instances.RGA_TombstoneFree.RGA_HEnum_Discharge
import Sal.ConditionedMRDTs.MRDT_Instances.RGA_TombstoneFree.RGA_MergeCanon_Fix
import Sal.ConditionedMRDTs.MRDT_Instances.RGA_TombstoneFree.RGA_BranchCanon
import Sal.ConditionedMRDTs.MRDT_Instances.RGA_TombstoneFree.RGA_FiltEq
...
theorem canonAnc_dead_eq_zero (F : List op_t) :
...
theorem liveSub_nil_all_dead (s : concrete_st) :
...
theorem first_live_split (s : concrete_st) :
...
theorem home_dead_F_dead (Cfg : Sal.Emulation.Configuration RGACondSig.toCRDTSig)
    (E : Set op_t)
    (hids0 : ∀ x ∈ E, x.1 ≠ 0)
    ...
    (ρh : List op_t) (hρhsub : ∀ x ∈ ρh, x ∈ E)
    (hinvh : CanonInv ρh (applySeqR init_st ρh))
    (t r e a : ℕ) (p : List ℕ) (htE : (t, r, app_op_t.Ins e p a) ∈ E)
    (hdepsh : ∀ z : op_t, z ∈ E → z ≠ (t, r, app_op_t.Ins e p a) →
        DepC Cfg E z (t, r, app_op_t.Ins e p a) → z ∈ ρh) :
    ∀ c ∈ a :: p, contains (applySeqR init_st ρh) c = false → ¬ survP F c := by
...
theorem rga_hbridge_discharged
    ...
    (h₀OK : CanonFoldOK [] init_st ρ₀) (h₁OK : CanonFoldOK [] init_st ρ₁)
    ...
    ∀ (t r e a : ℕ) (p : List ℕ),
      ...
      CanonBirthBridge (applySeqR init_st ρ₀) (ρ₀ ++ π₀)
        (birthAnc (applySeqR init_st ρ₀) (applySeqR init_st ρ₁) (applySeqR init_st ρ₂) t)
        (a :: p) := by
  ...
  have hUr : respects U (loOnA RGACondSig Cfg (ev₁ ∪ ev₂)) :=
  ...
  have hinv1 : CanonInv ρ₁ (applySeqR init_st ρ₁) := by
    have h := RGACanonConvergence.canon_fold ρ₁ [] init_st
      RGACanonConvergence.canonInv_init h₁OK
    rwa [List.nil_append] at h
  have hinv2 : CanonInv ρ₂ (applySeqR init_st ρ₂) := by
    have h := RGACanonConvergence.canon_fold ρ₂ [] init_st
      RGACanonConvergence.canonInv_init h₂OK
    rwa [List.nil_append] at h
  have hcatOK : CanonFoldOK [] init_st (ρ₀ ++ π₀) :=
    Sal.ConditionedMRDTs.RGACorrectedResidual.canonFoldOK_concat ρ₀ [] init_st π₀ h₀OK hπOK
  have hinvF : CanonInv (ρ₀ ++ π₀) (applySeqR init_st (ρ₀ ++ π₀)) := by
    have h := RGACanonConvergence.canon_fold (ρ₀ ++ π₀) [] init_st
      RGACanonConvergence.canonInv_init hcatOK
    rwa [List.nil_append] at h
  have hcmF : CanonMatch (ρ₀ ++ π₀) (applySeqR init_st (ρ₀ ++ π₀)) :=
  ...
  have hD : ∀ j, survivors (applySeqR init_st ρ₀) (applySeqR init_st ρ₁)
      (applySeqR init_st ρ₂) j = contains (applySeqR init_st (ρ₀ ++ π₀)) j := by
    intro j
    obtain ⟨hI0, hD1I, hD2I, hD01, hD02, hIu, hDu⟩ := hcaus j
    have hdomm := merge_domain_clause (applySeqR init_st ρ₀) (applySeqR init_st ρ₁)
      (applySeqR init_st ρ₂) ρ₀ π₀ ρ₁ ρ₂ j
  ...
  have hbridge_from_home : ∀ ρh : List op_t,
      (∀ x ∈ ρh, x ∈ ev₁ ∪ ev₂) →
      CanonFoldOK [] init_st ρh →
      (t, r, app_op_t.Ins e p a) ∈ ρh →
      (∀ z : op_t, z ∈ ev₁ ∪ ev₂ → z ≠ (t, r, app_op_t.Ins e p a) →
          DepC Cfg (ev₁ ∪ ev₂) z (t, r, app_op_t.Ins e p a) → z ∈ ρh) →
      birthAnc (applySeqR init_st ρ₀) (applySeqR init_st ρ₁) (applySeqR init_st ρ₂) t
        = anc (applySeqR init_st ρh) t →
      CanonBirthBridge (applySeqR init_st ρ₀) (ρ₀ ++ π₀)
        (birthAnc (applySeqR init_st ρ₀) (applySeqR init_st ρ₁) (applySeqR init_st ρ₂) t)
        (a :: p) := by
    intro ρh hρhsub hhOK htρh hdepsh hba
    have hinvh : CanonInv ρh (applySeqR init_st ρh) := by
      have h := RGACanonConvergence.canon_fold ρh [] init_st
    ...
    have hlcPath : IsAncPath (applySeqR init_st ρh) t
        (liveSub (applySeqR init_st ρh) (a :: p)) := hlc.2.2
    cases hLS : liveSub (applySeqR init_st ρh) (a :: p) with
    | nil =>
      -- birth anchor 0: the whole record is home-dead, canonAnc collapses to the root
      rw [hLS] at hlcPath
      have hanc0 : anc (applySeqR init_st ρh) t = 0 := hlcPath
      have hAllDead : ∀ c ∈ a :: p, ¬ survP (ρ₀ ++ π₀) c := fun c hc =>
        hdead c hc (liveSub_nil_all_dead (applySeqR init_st ρh) (a :: p) hLS c hc)
      rw [hba, hanc0]
      refine ⟨?_, ?_⟩
      · intro h0live
    ...
    | cons bw rest =>
      ...
      rw [hLS] at hlcPath
      have hancbw : anc (applySeqR init_st ρh) t = bw := hlcPath.1
      have hbwlive : contains (applySeqR init_st ρh) bw = true := hlcPath.2.1
      have hbw0 : bw ≠ 0 := by
        rintro rfl
        rw [hinvh.1] at hbwlive
        exact absurd hbwlive (by simp)
      have hbaBW : birthAnc (applySeqR init_st ρ₀) (applySeqR init_st ρ₁)
          (applySeqR init_st ρ₂) t = bw := hba.trans hancbw
      obtain ⟨rcPre, rcSuf, hsplit, hpreDead, _⟩ :=
        first_live_split (applySeqR init_st ρh) (a :: p) bw rest hLS
      have hpreDeadF : ∀ c ∈ rcPre, ¬ survP (ρ₀ ++ π₀) c := fun c hc =>
        hdead c (by rw [hsplit]; exact List.mem_append_left _ hc) (hpreDead c hc)
      have hsv : survivors (applySeqR init_st ρ₀) (applySeqR init_st ρ₁)
          (applySeqR init_st ρ₂) t = true := by
        rw [hD t]
        exact (hcmF.1 t).mpr hsvF
      have hin : contains (applySeqR init_st ρ₀) (birthAnc (applySeqR init_st ρ₀)
          (applySeqR init_st ρ₁) (applySeqR init_st ρ₂) t) = true →
          ∃ cw, IsAncPath (applySeqR init_st ρ₀) (birthAnc (applySeqR init_st ρ₀)
            (applySeqR init_st ρ₁) (applySeqR init_st ρ₂) t) cw ∧
      ...
      exact canonBirthBridge_via_branchCanon (applySeqR init_st ρ₀) (applySeqR init_st ρ₁)
        (applySeqR init_st ρ₂) (ρ₀ ++ π₀) (applySeqR init_st (ρ₀ ++ π₀)) t
        (a :: p) rcPre rcSuf
  ...
  by_cases h0t : contains (applySeqR init_st ρ₀) t = true
  · -- home = the LCA
    have hsv0 : survP ρ₀ t := (hinv0.2.2.1 t).mp h0t
    obtain ⟨r', e', p', a', hm⟩ := hsv0.1
    have heq : ((t, r', app_op_t.Ins e' p' a') : op_t) = (t, r, app_op_t.Ins e p a) :=
      hopEq (hsub0 _ hm) htE rfl
    rw [heq] at hm
    have htI : (t, r, app_op_t.Ins e p a) ∈ ev₁ ∩ ev₂ := hmemρ _ hm
    have hba : birthAnc (applySeqR init_st ρ₀) (applySeqR init_st ρ₁)
        (applySeqR init_st ρ₂) t = anc (applySeqR init_st ρ₀) t := by
      unfold birthAnc
      rw [if_pos h0t]
  ...
  · by_cases h1t : contains (applySeqR init_st ρ₁) t = true
    · -- home = branch 1
