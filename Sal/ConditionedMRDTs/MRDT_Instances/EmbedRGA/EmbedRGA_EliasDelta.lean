...
Order-coding note I5, machine-validated: because `embed_ra_linearizable3`
is parametric in the code `Γ`, upgrading the per-level coordinate cost from
`2 log₂ δ + 1` to `log₂ δ + O(log log δ)` is pure plug-in — this file's
theorem has zero new proof content. Together with `unaryCode` and
`binaryCode`, the datatype now has three verified encodings on one proof.
-/
