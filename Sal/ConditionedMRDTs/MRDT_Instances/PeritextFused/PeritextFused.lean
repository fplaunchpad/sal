...
separate mark store (`PeritextTF/`).  This is the tombstone-free/live corner of the
...
`Mark → Gravity` (see `PeritextFused_Read.lean`), never stored.

## What is inherited (Phase 1, this file)

Because the element is opaque, **nothing** about convergence is re-proved here.
`peritextFused_ra_linearizable_up_to_eq` is a one-line instantiation of the RGA
capstone `rga_tombstone_free_ra_linearizable3_eq` at `α := PeritextElt`.  Per-version
...
delivery).  The product design `PeritextTF/` needed **two** contracts: the RGA's
...
namespace Sal.ConditionedMRDTs.PeritextFused
...
noncomputable abbrev PeritextFusedSig : ConditionedMRDTSig := RGACondSig' PeritextElt
...
This is `rga_tombstone_free_ra_linearizable3_eq` at `α := PeritextElt`, verbatim.
...
(`PeritextFused_Read.lean`). -/
theorem peritextFused_ra_linearizable_up_to_eq
  ...
  rga_tombstone_free_ra_linearizable3_eq (α := PeritextElt) hHD C hReach

/-! ## §4  Axiom audit -/

#print axioms peritextFused_ra_linearizable_up_to_eq

end Sal.ConditionedMRDTs.PeritextFused
