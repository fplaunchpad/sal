...
The Phase-1 capstone (`PeritextFused.lean`) certifies state convergence up to `≈`.
...
3. **The genuine intent theorem — the whole point.**  `render_id_active_iff_between`
   ...
   earlier text (`PeritextTF/MarkIntent.lean`, the retracted `mark_*_no_leak`).  Here
...
namespace Sal.ConditionedMRDTs.PeritextFused.Read
...
(`PeritextTF/MarkIntent.lean`).  Here the boundary is a live node, so a character's
...
end Sal.ConditionedMRDTs.PeritextFused.Read
...
namespace Sal.ConditionedMRDTs.PeritextFused.ReadSPOT

open Sal.ConditionedMRDTs.PeritextFused
open Sal.ConditionedMRDTs.PeritextFused.Read
...
(`PeritextTF/MarkIntent.lean`, the retracted `mark_*_no_leak`); here it cannot. -/
...
end Sal.ConditionedMRDTs.PeritextFused.ReadSPOT
