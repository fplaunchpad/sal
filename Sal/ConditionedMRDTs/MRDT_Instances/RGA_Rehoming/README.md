...
> fully general, but the design is **sequential-spec-refuted at the `do`
> level** (`RGA_SeqSpec_Refuted.lean` here, and
> `tombstone_free_violates_delete_order` in the model tree): a single-replica
> delete reorders survivors, so the datatype does not implement the naive
> text buffer, and fused Peritext (its pure instantiation) inherits the
> residual at the render. It is retained as the framework's generality
> stress test and the delete-order countermodel. The canonical sequence RDT
> is the embedded-chain family (`../EmbedRGA/`, `../SidedRGA/`).
