...
/-!
# The TWO-SIDED merge-linearization bridge for the tombstone-free RGA

This file extends the single-sided bridge in `RGA_MergeLinearization.lean` to
both branches.  Target (up to observational `eq`):
...
* **eq-extensionality.**  `BranchInv2 l a b p` is the exact
  ...
  `BranchInv2 l a b p → eq (merge l a b) p`, kernel-clean.  This is the
...
* **interleave-order independence.**  `merge_fold_indep` lifts the
  bridge from one reference interleave `π₀` to any `loOnA`-respecting interleave `π`,
  via the imported convergence engine (`eq_convergence`, start state `l`),
  kernel-clean, given the same swap oracle the convergence headline already
  consumes.  The engine applies with no change, since it is start-state-generic.

* **assembled headline.**  `eq_merge_two_sided` composes the two,
...
Point 2's generalization is mechanized and kernel-clean:
`branchInv_doDel_crossBranch` (§1b) preserves `BranchInv l` under a
...
What blocks `hThread` in general is not point 2, but two narrower facts (the exact
stuck goals are spelled out below):
...
it (§1 extensionality, §2 interleave-independence, §3 assembly) is kernel-clean,
conditional on `hThread` + `hSwap` only.
-/
...
/-- **eq-extensionality.**  `BranchInv2 l a b p` is exactly
...
/-! ## §1b  The resolve-vs-climb reconciliation and the
...
/-! ## §2  Interleave-order independence
...
**Approach.**  `applySeqR l (Ea ++ Eb) = applySeqR a Eb` (`applySeqR_append`,
`a = applySeqR l Ea`), so an invariant can be threaded through folding `Eb` over
`a`.

**Reduction of the original-node clauses.**  Given
  (D)  `domain (applySeqR l π₀) = survivors l a b`, and
  (B)  `RGAMergeLinearization.BranchInv l (applySeqR l π₀)`,
`BranchInv2`'s dom clause is (D), its el clause is I2 of (B) (original nodes
keep `el l`), and its anc clause for an original survivor `k` coincides with I4
of (B), because `birthAnc l a b k = anc l k` and, by (D), the stop-set
`survivors l a b` is `domain (applySeqR l π₀)`.  So the original-node content
reduces to single-sided (B) + (D).

**Branch-new survivors.**  `BranchInv2`'s anc clause also quantifies over
...
nodes, so (B) says nothing here.  This needs a strengthened invariant clause:
for a branch-new survivor `k`, `anc (fold) k = climb (anc l) (dom (fold))
(branch-birth-anchor of k)`.  For `a`-new `k` this holds at the fold start `a`
by `climb_fixpoint` (`anc a k` is `0`-or-live-in-`a` ⊆ `a`'s stop set), and must
be preserved across every `Eb` step; for `b`-new `k` it must be established at
`k`'s `Eb`-`Ins` birth (its combined-state insert anchor `v = resolve(current,
path)` vs. its branch anchor `anc b k = resolve(b, path)` differ, and must be
shown to climb to the same survivor).

**Cross-branch stale-path `Del`.**  Establishing (B) via
`RGAMergeLinearization.branchInv_triple_fold` needs `π₀ = Ea ++ Eb` to be a
`GoodBranchFold l l`, whose `Del` steps require
`BranchStepOK l s (Del pre x) ≡ accurate (Del pre x) s` at the running prefix
state `s`.  An `Eb`-`Del` of an `l`-node `x` carries a `b`-relative path; over
`s = a ++ (Eb`-prefix`)` that path is stale exactly when `a` and `b` delete
overlapping or nested `l`-nodes, so `accurate (Del pre x) s` fails and
`branchInv_doDel` does not apply.  (When `x` is already deleted by `a`, the
`Del` is stale-absent; when `x` is live but an ancestor of `x` was deleted by
`a`, `x`'s carried path lists a node `a` removed.)
...
what is needed is a generalized `Del`-preservation lemma.  This is
`branchInv_doDel_crossBranch` (§1b, kernel-clean):
...
**The residual on stale paths.**  If `Eb` deletes an `l`-ancestor of `x` before
`x`, RGA rehoming shortens `x`'s carried path to a proper subchain of its
`l`-ancestor chain, so `IsAncPath l x pre` is false and `resolve_climb_lchain`
does not apply, even though `resolve s pre = anc s x` still holds.  Concrete
witness: `l = 0←1←2←3`, `Eb = [Del [1] 2, Del [1] 3]`: the `Del` of `3` carries
`[1]`, but `3`'s `l`-chain is `[2,1]`; both `resolve`/`climb` land on `1`.
Closing this needs a subchain generalization of `resolve_climb_lchain` (`pre` =
the `s`-live subchain of `x`'s `l`-ancestors) plus a fold-level invariant
certifying the `Eb` paths are exactly such subchains: the imported
`Faithful`/`ChainFaithful` layer (`RGA_ConditionedConvergence` §2–3), not
single-sided `BranchInv` alone.

**Summary.**  The cross-branch `Del`-preservation reduction is closed
(`branchInv_doDel_crossBranch`, kernel-clean).  The two-sided merge = fold
identity does not yet close by pure single-sided reuse: the residue is
branch-new survivor tracking plus `domain = survivors`, and supplying `hres` at
fold time for `b`-nested-delete-shortened paths.  Everything downstream of
`hThread` (extensionality §1, interleave-independence §2, assembly §3) is
kernel-clean.  Merge and fold agree on every cross-branch scenario
(PBT-confirmed, including the witness above): the residue is the
shortened-path bookkeeping lemma and branch-new survivor tracking, not a
divergence. -/
