...
import Sal.ConditionedMRDTs.MRDT_Instances.EmbedRGA.EmbedRGA_EpochDiamond
...
| **BudgetCart** (per-replica budgets) | `BCart_ra_linearizable3_eq`; safety: `bcart_version_inv_gated` (gated on `CausalCanonical`) |
...
The mergeable queue is a genuinely conditioned instance proved by a **direct
Join Lemma** (`q_join_at`): concurrent enqueues form a non-commuting clique,
so no `rc` assignment exists and the flat VC engine is structurally
unavailable; instead Peepul's three-way merge is itself exhibited as the
linearization witness at every merge, under the honest-delivery contract
`QHonest` (every dequeue names an observed enqueue, discharged by the
`applicable` head-check, `qHonest_of_applicable`).

The bounded counter is a genuinely conditioned instance carrying a **safety**
capstone: `bc_version_inv` proves the escrow invariant (hence
`bc_value_nonneg`, the bound) at every version of every reachable
configuration whose history satisfies the formal client contract
