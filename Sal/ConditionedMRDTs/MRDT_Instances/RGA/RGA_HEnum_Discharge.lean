import Sal.ConditionedMRDTs.MRDT_Instances.RGA.RGA_Skeleton3
import Sal.ConditionedMRDTs.MRDT_Instances.RGA.RGA_GenDisc_Peel
...
def rgaHonJ (vis : op_t α → op_t α → Prop) (events : Set (op_t α)) : Prop :=
  ∃ Cfg : Sal.Emulation.Configuration (RGACondSig α).toCRDTSig,
    (∀ a b : op_t α, Cfg.vis a b ↔ (vis a b ∧ a ∈ events ∧ b ∈ events)) ∧
    GenDisc2C Cfg events ∧
    (∀ o ∈ events, o.1 ≠ 0) ∧
    (∀ a b : op_t α, a ∈ events → b ∈ events → vis a b → a.1 < b.1) ∧
...
theorem rga_hEnum_discharged :
  ...
  · -- K1: the delta discipline continued from the LCA fold
    have hcl1' : ∀ a b : op_t α, Cfg.vis a b → b ∈ ev₁ → a ∈ ev₁ :=
      fun a b h hb => hcl1 a b ((hviseq a b).mp h).1 hb
    have hcl2' : ∀ a b : op_t α, Cfg.vis a b → b ∈ ev₂ → a ∈ ev₂ :=
    ...
    have hπrA : respects π₀ (loOnA (RGACondSig α) Cfg (ev₁ ∪ ev₂)) :=
