...
namespace Sal.ConditionedMRDTs.GoodConfig3NF
...
def GoodConfig3NF (E : EqEquiv D) (W : Op D.AppOp → D.State → Prop)
  ...
  Sal.ConditionedMRDTs.GoodConfig3 C ∧
  ∀ (v : Sal.ConditionedMRDTs.Version) (s : QState D E) (Ev : Set (Op D.AppOp)),
    C.ver v = some (s, Ev) →
...
theorem isRALinearizable3_of_goodNF (E : EqEquiv D) (W : Op D.AppOp → D.State → Prop)
  ...
  Sal.ConditionedMRDTs.isRALinearizable3_of_good h.1
...
theorem goodConfig3NF_init (E : EqEquiv D) (W : Op D.AppOp → D.State → Prop)
    ...
    GoodConfig3NF E W hP hC hA
      (Sal.ConditionedMRDTs.initConfig (QSig E W hP hC hA) trivial) := by
  refine ⟨Sal.ConditionedMRDTs.goodConfig3_init trivial, ?_⟩
  intro v s' E' hv
  have hver : (Sal.ConditionedMRDTs.initConfig (QSig E W hP hC hA) trivial).ver v
...
theorem goodConfig3NF_createReplica (E : EqEquiv D) (W : Op D.AppOp → D.State → Prop)
    (hP : InvPres D W) (hC : CongVC D E) (hA : InvInvVC D E W)
    {C C' : Sal.ConditionedMRDTs.Configuration (QSig E W hP hC hA)}
  ...
  refine ⟨Sal.ConditionedMRDTs.goodConfig3_createReplica h_fresh hL hvis hver h.1, ?_⟩
  intro w s' E' hw
  rw [hver] at hw
  exact isCanonicalStateNF_congr E W hP hC hA
    (fun a _ b _ => by rw [Sal.ConditionedMRDTs.core_vis, Sal.ConditionedMRDTs.core_vis, hvis])
    (h.2 w s' E' hw)
...
theorem goodConfig3NF_apply (E : EqEquiv D) (W : Op D.AppOp → D.State → Prop)
    ...
    {v : Sal.ConditionedMRDTs.Version} {s : QState D E} {ev : Set (Op D.AppOp)}
    {vnew : Sal.ConditionedMRDTs.Version}
    ...
    (h : GoodConfig3NF E W hP hC hA C) :
    GoodConfig3NF E W hP hC hA C' := by
  refine ⟨Sal.ConditionedMRDTs.goodConfig3_apply h_head h_ver h_fresh_t h_vnew hL hvis hver h.1, ?_⟩
  ...
  have h_vis_old : ∀ (E' : Set (Op D.AppOp)), (∀ x ∈ E', x ∈ C.events) →
    ...
    rw [Sal.ConditionedMRDTs.core_vis, Sal.ConditionedMRDTs.core_vis, hvis]
    constructor
    · exact Or.inl
    · rintro (hab | ⟨_, rfl⟩)
      · exact hab
  ...
  · rw [hwn, hver_new, Option.some.injEq, Prod.mk.injEq] at hw
    ...
    have h_ext := isCanonicalStateNF_extend E W hP hC hA hInvCong (C'.core) ev s e
      hgenW he_not_ev
      (fun x hx => by
        rw [Sal.ConditionedMRDTs.core_vis, hvis]; exact Or.inr ⟨hx, rfl⟩)
      (fun x hx => by
        rw [Sal.ConditionedMRDTs.core_vis, hvis]
        rintro (hex | ⟨he_ev, _⟩)
...
theorem goodConfig3NF_merge_of_canonical (E : EqEquiv D) (W : Op D.AppOp → D.State → Prop)
    (hP : InvPres D W) (hC : CongVC D E) (hA : InvInvVC D E W)
    {C C' : Sal.ConditionedMRDTs.Configuration (QSig E W hP hC hA)}
    {r₁ : Sal.Emulation.Replica} {v₁ v₂ vm : Sal.ConditionedMRDTs.Version}
  ...
  have h_sameNF : ∀ (E' : Set (Op D.AppOp)) (s' : QState D E),
    ...
    fun E' s' hcs => isCanonicalStateNF_congr E W hP hC hA
      (fun a _ b _ => by rw [Sal.ConditionedMRDTs.core_vis, Sal.ConditionedMRDTs.core_vis, hvis]) hcs
  intro w s' E' hw
  by_cases hwn : w = vm
  · rw [hwn, hver_new, Option.some.injEq, Prod.mk.injEq] at hw
    rw [← hw.1, ← hw.2]
...
theorem goodConfig3NF_merge (E : EqEquiv D) (W : Op D.AppOp → D.State → Prop)
    ...
    (hJoinNF : EqJoinLemma3C_NF D E W)
    {C C' : Sal.ConditionedMRDTs.Configuration (QSig E W hP hC hA)}
    {r₁ : Sal.Emulation.Replica} {v₁ v₂ vT vm : Sal.ConditionedMRDTs.Version}
    {s₁ s₂ sT : QState D E} {ev₁ ev₂ evT : Set (Op D.AppOp)}
    (h_head₁ : C.head r₁ = some v₁)
    (h_ver₁ : C.ver v₁ = some (s₁, ev₁)) (h_ver₂ : C.ver v₂ = some (s₂, ev₂))
    (h_lca : Sal.ConditionedMRDTs.IsLCA C.parents v₁ v₂ vT)
...
theorem goodConfig3NF_of_reachF (E : EqEquiv D) (W : Op D.AppOp → D.State → Prop)
    ...
    (hBA : ∀ {C₀ C₁ : Configuration (QSig E W hP hC hA)}
      ...
      {v : Sal.ConditionedMRDTs.Version} {sh : QState D E} {evh : Set (Op D.AppOp)},
...
theorem RA_linearizable_up_to_eq_NF (E : EqEquiv D) (W : Op D.AppOp → D.State → Prop)
    ...
    (hInvCong : ∀ {s s' : D.State}, E.eqv s s' → D.Inv s → D.Inv s')
    (hJoinNF : EqJoinLemma3C_NF D E W)
    (hBA : ∀ {C₀ C₁ : Sal.ConditionedMRDTs.Configuration (QSig E W hP hC hA)}
      {t : Sal.Emulation.Timestamp} {r : Sal.Emulation.Replica} {o : D.AppOp}
      {v : Sal.ConditionedMRDTs.Version} {sh : QState D E} {evh : Set (Op D.AppOp)},
      (labeledTS3 (QSig E W hP hC hA)).ReachableFrom
        (Sal.ConditionedMRDTs.initConfig (QSig E W hP hC hA) trivial) C₀ →
      Sal.ConditionedMRDTs.Step3 (QSig E W hP hC hA) C₀ (Sal.ConditionedMRDTs.Label3.apply t r o) C₁ →
      C₀.head r = some v → C₀.ver v = some (sh, evh) →
      qapplicable E W hA (t, r, o) sh ∧
        (∀ s', D.applicable (t, r, o) s' → W (t, r, o) s'))
    (C : Sal.ConditionedMRDTs.Configuration (QSig E W hP hC hA))
...
end Sal.ConditionedMRDTs.GoodConfig3NF
