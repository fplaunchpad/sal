...
hypothesis. The four mutation rules have since been promoted from
SPOT-pinned perturbations to closed \lean{tableOf} equations, the
table-direct walk is literal (\lean{tableWalk\_tableOf}: the display
emitted run by run without materializing \lean{stateOf}), and the
\emph{sided} run table is complete (\code{SidedRunTable.lean}): the repr
iso both directions over labels, sides, and liveness, the two-case
comparator \lean{scmpTable\_eq\_keyLt} for every code, the O(1) typing
equation, the in-order walk with its literal \lean{stableWalk}, and the
all-R lockstep bridge (\lean{sTableOf\_liftR}: on an all-R set the sided
table is the one-sided table with the side bit stamped). The one open
face is the wire format (Open Question~\ref{oq:runtable}).
...
(task~\#73). One-sided (\code{RunTable.lean}): T-tail
(\lean{tail\_attachment}), T-repr as a genuine bijection, T-cmp
(\lean{cmpTable\_eq\_keyLt}), T-epoch (\lean{runTable\_epoch}), the four
T-mut rules promoted from SPOT-pinned perturbations to \emph{closed
equations} (\lean{tableOf\_insert\_split},
\lean{tableOf\_delete\_split}, \lean{tableOf\_coalesce},
\lean{tableOf\_materialize}; the general vanished-path cascade is
documented as the composite of materialization with insert splits,
deliberately not forced into one equation), and the literal table-direct
walk with its capstone \lean{tableWalk\_tableOf} (the display emitted
run by run, \lean{stateOf} never materialized: the formal face of the
O(display) reading claim). Sided (\code{SidedRunTable.lean}): the full
mirror --- repr iso both directions over labels, sides, and liveness
(timestamps deliberately excluded, the same post-epoch reason), the
two-case comparator \lean{scmpTable\_eq\_keyLt} for every code with the
offset-vs-tail exit-side rule, congruence, the O(1) typing equation, the
in-order walk and its literal \lean{stableWalk}, and the all-R lockstep
bridge \lean{sTableOf\_liftR}/\lean{swalk\_liftR}: on an all-R set the
sided table \emph{is} the one-sided table with the side bit stamped.
What remains is one engineering face: task~\#104's serializer turned the
...
run table, sided (full mirror: iso, comparator, walks, all-R lockstep) & \lean{sTableOf\_sStateOf}, \lean{scmpTable\_eq\_keyLt}, \lean{stableWalk\_sTableOf}, \lean{sTableOf\_liftR} & \lean{Sal/MRDTs/RGA\_Embed/SidedRunTable.lean} \\
