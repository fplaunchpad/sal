...
\date{July 19, 2026\\[2pt] \small one document consolidating three
earlier notes (of July 7, July 14, and July 16--17, 2026),\\
\small extended July 18--19 with the runtime arc (virtual LCAs, commit
GC, the stability VC)\\
\small and the storage layer (re-coding, compaction, the run table)}
...
layer with its measurements (\S\ref{sec:storage}). One editorial rule
...
runtime could never check; it is dischargeable by evidence the commit
DAG already contains. An event concurrent with $s \in S$ was minted by
some replica $j$ before $j$ saw $s$; the evidence that $j$ saw $s$ is a
commit $c_j$ with $s \in E(c_j)$; the concurrent event precedes $c_j$
at $j$. So if $v$ has absorbed such an evidence commit \emph{for every
replica} ($v$ has heard from everyone since $S$), all concurrency of
$S$ is inside $E(v)$
(\lean{settledAt\_of\_evidence}). This is causal stability in the sense
...
on downward closure of event sets. Two operational sharpenings from the
...
The Lean mechanization of this subsection is owed, and the
implementation pass fixed what the theorem must say: a representation
isomorphism stated over labels, sides, and liveness (post-epoch,
\emph{not} timestamps: rank ordinals deliberately forget time, so a
timestamp-preserving iso would be false after the first epoch), the
tail-attachment lemma as the load-bearing kernel, comparator and walk
agreement, six mutation-commutes-with-canonicalization lemmas (with
coalesce, the piece a paper proof would most plausibly hand-wave), and
the epoch composition, the only clause with a stability hypothesis.
Recorded as Open Question~\ref{oq:runtable}.

\subsection{The cross-system benchmark}
\label{sec:crossbench}

The storage layer's internal measurements say how far the design's own
representations sit from each other; the external question is where the
shipped artifact sits against production CRDT libraries. The benchmark
(\code{benchmarks/}, task \#98) runs our runtime (the unverified ESM
transliteration of the verified embed kernel, flipped Elias-$\delta$
code) against Yjs 13.6, Automerge 3.3 (wasm), Loro 1.13 (wasm), and
list-positions 2.0, on three workloads: the sequential trace replay of
the previous subsections applied one character at a time, a
deterministic two-replica concurrent session (seeded, byte-identical op
stream to every system), and a delete-heavy churn cycle. Every cell
comes from a run of the checked-in scripts on one machine (node
v26.3.1, Apple M4 Pro); nothing is quoted from literature, and each
system's convergence and final text are gated before any number is
reported.

\paragraph{Fair play: what each column contains.} Save sizes across
CRDT systems are not comparable without saying what a save
\emph{contains}, so every row is labeled. Automerge's \code{save} is
the \emph{full change history}, compressed (the library has no
state-only mode). Loro's \code{snapshot} carries state plus full
history; its \code{shallow-snapshot} drops history at the current
frontiers and is state-only. Yjs's update encoding drops deleted
content but cannot drop tombstone \emph{id structure} (v2 is the same
content, run-length compressed). list-positions saves live characters
plus position metadata as JSON. Ours saves \emph{live state only}
(delete is pure removal). Symmetrically, our side is three
clearly-labeled columns: \emph{as shipped} (measured: the datatype's
JSON, plus a \emph{binary estimate}, a defined packed encoding computed
from the same state, honest about the fact that no packed encoder is
shipped); \emph{with compaction} (measured, shipped code: the state
after \code{compactEliasDelta} at a settled cut, legitimate on these
workloads since replay and churn are single-writer and the session is
fully synced at the end); and the \emph{run-table projection}
(measured-in-model, \textbf{not shipped}: the accounting of
\S\ref{sec:runtable} executed on the same trace, order-metadata bits
plus UTF-8 text). The projection is anchored to the shipped code
bit-for-bit: the model's chain accounting reproduces the shipped
state's coordinate bits exactly (34{,}660{,}055 bits before and
18{,}296{,}270 after compaction on \code{friendsforever\_flat}), so the
column measures the representation, not a different datatype.

\paragraph{The condensed save-size comparison} (bytes per live
character, final state of each sequential trace; ``history'' rows carry
the whole edit log, ``state'' rows do not):

\begin{center}\small
\setlength{\tabcolsep}{4.5pt}
\begin{tabular}{@{}llrrrr@{}}
\toprule
system, variant & contains & \code{ff\_flat} & \code{cs\_flat} &
  \code{seph} & \code{am-paper} \\
\midrule
ours, as shipped (binary est.) & state & 207.2 & 315.5 & 376.3 & 292.5 \\
ours, with compaction (binary est.) & state & 111.5 & 188.3 & 119.1 & 164.4 \\
ours, run-table \emph{projection} & state (model) & 3.8 & 3.8 & 3.9 & 4.0 \\
Yjs update-v1 & state + tombstone ids & 3.8 & 4.4 & 6.0 & 3.0 \\
Yjs update-v2 & same, run-length coded & 1.9 & 2.0 & 2.4 & 1.5 \\
Automerge save & full history & 1.3 & 1.2 & 3.6 & 1.2 \\
Loro snapshot & state + full history & 2.9 & 3.1 & 5.9 & 2.4 \\
...
\noindent (Traces abbreviated: \code{friendsforever\_flat},
\code{clownschool\_flat}, \code{seph-blog1}, \code{automerge-paper};
final documents 21k, 21k, 57k, 105k characters.) Read bottom-up: the
as-shipped absolute-coordinate representation is two orders of
magnitude above production, compaction roughly halves it, and the
run-table projection lands at 3.8 to 4.0 bytes per character, within
1.4$\times$ of Yjs update-v1 on every trace but still 3 to 6$\times$
above the smallest state-only production saves (Loro shallow-snapshot,
Yjs v2). The projection closes the representation gap to rough parity
with Yjs v1; it does not win save size outright, and it must never be
read as a shipped number.

\paragraph{The honest headline.} Where we lose, as shipped: per-char
apply, by three to four orders of magnitude. Median 347 to 364~$\mu$s
on the 21k-character documents, 1.66~ms at 57k, 3.16~ms at 105k, and
the trend is the diagnosis: the persistent \code{apply} returns a fresh
Map, an $O(\text{live set})$ copy per op, so cost grows linearly with
document size. This is \emph{interface-inherited} (the persistent-state
datatype interface the verified semantics is stated over), not order
machinery; Automerge sits at a flat 24 to 27~$\mu$s and
Yjs/Loro/list-positions at a flat 0.5 to 1.7~$\mu$s, and a transient or
batched apply path would move ours toward the microsecond class without
touching the verified semantics. Load (JSON parse plus the coordinate
sort) is 31 to 253~ms against Loro's sub-millisecond snapshot load,
though within range of Automerge's full-history load. Retained memory
is small (2 to 12~MB: absolute coordinates share prefixes structurally
as cons strings, so the bit cost materializes only at serialization),
paid for as allocation churn during replay.

Where we are competitive or winning, as shipped: merge/sync. The
unique-LCA three-way merge under the head-sync runtime syncs at
73~$\mu$s median (frequent-sync preset) and 731~$\mu$s (bulk), which is
Yjs-level (90~$\mu$s, 1.13~ms) and 20 to 25$\times$ faster than
Automerge and Loro in these sessions, with commit GC running outside
the timed window at millisecond totals. The honest caveat: these are
small documents (1.8k and 3.6k characters), and our merge is
$O(\text{live set})$, so the numbers do not extrapolate to 100k-char
documents. And storage on delete: our save \emph{never grows on
delete}; it shrinks by construction, since delete is record removal.

\paragraph{The churn table} (5 cycles of insert 2000 then delete 1800
characters, then a final 200-char insert; save bytes after selected
phases; the question per row is whether the save \emph{grows across a
delete phase}):
...
\toprule
system, variant & c1-ins & c1-del & c5-ins & c5-del & final &
  grows on delete? \\
\midrule
ours (binary est.) & 22278 & 2191 & 72322 & 25891 & 31596 & never \\
ours, compacted (binary est.) & 11576 & 1090 & 18884 & 6524 & 7882 & never \\
...
Automerge's save grows by roughly 3.4~KB across \emph{every} delete
phase (history-carrying, monotone by design), and Loro's full snapshot
likewise; Yjs never grows but cannot shed tombstone structure, so it
retains 165~KB (v1) for those 1200 characters against our compacted
7.9~KB and Loro's shallow 3.9~KB. Ours shrinks on every delete phase in
every variant. This is the storage-on-delete axis of the retention
taxonomy (\S\ref{sec:related}) measured live: tombstone-freedom is not
a benchmark trick, it is the design's semantics showing up as the only
save that gets smaller when the document does.

Methodology caveats, stated so the numbers cannot be over-read: timer
overhead (30 to 60~ns per op) is not subtracted and biases the
sub-microsecond medians; Automerge is driven one change per character
(the automerge-perf convention; batching would lower its cost and
history size); list-positions is a positions library rather than a full
CRDT, and its sync row is its documented op-log integration; heap
numbers are not comparable across the wasm boundary; and cross-system
merged \emph{order} may legitimately differ on concurrent insertions,
so only intra-system convergence is gated.
...
\begin{openq}[evidence certificates for SettledAt]
\label{oq:evidence}
The stability gate (\S\ref{sec:stability}) is discharged by evidence
commits: the compacting version has heard from every replica since the
cut. Two operational gaps remain. Open membership: the evidence
quantifies over a known replica set, and replica creation after a
compaction is policy-gated rather than derived; a model where the
replica set itself is a grow-only MRDT could make the gate a theorem.
And certificate transport: can a replica hand a peer a compact,
checkable certificate of SettledAt (a frontier vector plus the
evidence commits) so the callback can fire at nodes that did not
compute the frontier themselves? The commit-shaped-not-event-shaped
erratum constrains the answer: the certificate must name commits, since
an event-set summary can miss the pull that carries no new events yet
opens the gate.
\end{openq}
...
linearized per replica (the v1 runtime refuses compaction off the
newest epoch), and VC-S6 gives observational coherence for nested cuts
on one lineage. Concurrent \emph{divergent} compactions are the open
protocol half: two replicas compact at incomparable cuts $S$, $S'$, and
their states must still merge. The conjectured shape is an epoch DAG
...
\begin{openq}[the run-table theorems]
\label{oq:runtable}
The run table (\S\ref{sec:runtable}) is machine-validated and its Lean
bill is fixed by the implementation pass: \emph{T-repr}, the
representation isomorphism between canonical tables and kept trees,
stated post-epoch over labels, sides, and liveness, deliberately not
timestamps (rank ordinals forget time; a timestamp-preserving statement
is false after the first epoch); \emph{T-tail}, the tail-attachment
lemma, proved once and consumed by the rest; \emph{T-cmp}, the two-case
path comparator equals the fold's key order, parametric in the tie
oracle exactly as the chain comparator is; \emph{T-walk}, the
structural walk equals the display sequence; \emph{T-mut}, six
mutation-commutes-with-canonicalization simulation lemmas, of which the
coalesce half is the one a paper proof would hand-wave and the
vanished-anchor rule must take the op's carried coordinate as input;
and \emph{T-epoch}, composition with the re-coding cluster, the only
clause with a stability hypothesis. Nothing conceptually new is
expected; the value is the usual one, that the mutation rules were
already corrected twice by hand-derivation during design. The
cross-system benchmark (\S\ref{sec:crossbench}) adds an engineering
face to the same item: a shipped run-table serializer would turn the
projection column from measured-in-model into measured, and a transient
or batched apply path would remove the interface-inherited per-op copy.
\end{openq}
...
SettledAt + evidence discharge (\S\ref{sec:stability}) & \lean{SettledAt}, \lean{settledAt\_of\_evidence} & \lean{Metatheory/Stability\_VC.lean} \\
stability VC bundle + metatheorem & \lean{StabilityVC}, \lean{stability\_simulation}, \lean{stability\_ra\_inherited} & \lean{Metatheory/Stability\_VC.lean} \\
...
run table (\S\ref{sec:runtable}; Lean owed, OQ~\ref{oq:runtable}) & --- & \code{whiteboard/litmus/run\_table\_measure.py} \\
\midrule
