...
signature's client-checkable $\app$ (or its accuracy analogue). Extracting
that shape generically, and with it a generic \emph{safety} metatheorem
($\Inv$ at every version from applicability-stability), is the natural
next step; the counter's counting argument shows the stability condition
is where the per-datatype content will sit.

\section{Open questions}
\label{sec:open}

The mechanization settles what the paper claimed; it also sharpens what
remains genuinely open. Seven questions, roughly in order of theoretical
depth:
