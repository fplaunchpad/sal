...
Peritext (tombstone-free) & \emph{composed} & \multicolumn{2}{l}{\lean{peritextTF\_ra\_linearizable\_up\_to\_eq} --- RGA${}\otimes{}$marks through the product kit; 1{,}064 lines total} \\
\bottomrule
\end{tabular}}
...
\lean{Sal/MRDTs/RGA\_Tombstone\_Free/doc/why-the-path-matters.pdf}, \S6).
...
(\lean{MRDT\_Instances/PeritextTF/}, capstone
\lean{peritextTF\_ra\_linearizable\_up\_to\_eq}: every version of every
...
It does (\lean{MRDT\_Instances/PeritextTF/MarkHonesty.lean}), and the way it
...
the read layer}: the render-intent theorems consume it
(\lean{MarkIntent.lean}), giving tombstone-free Peritext its ``formatting does
not leak'' guarantee at endpoint granularity --- a rendered endpoint is always
the document root or a member of its \emph{recorded chain}
(\lean{mark\_start\_no\_leak}, structural: \lean{resolve} returns a live
candidate or the root), which mark-anchor accuracy pins to the endpoint's true
issue-time ancestry, so formatting can only climb the anchored character's own
ancestry, never leak sideways; a non-root endpoint is moreover live at read
time (\lean{mark\_start\_live}, axiom-free). The strictly stronger read-time
claim --- that the surviving endpoint is a genuine ancestor \emph{at the read
state} --- needs the RGA's recorded-path-faithfulness invariant lifted along
the product LTS, the one Peritext-specific piece left. The lesson lifts
cleanly: a component's honesty
discipline is load-bearing exactly where the \emph{coupled read} depends on
it, not where convergence does --- and the read-coupled boundary carries
honesty precisely because the honesty predicate lives free of the
signature's componentwise $\app$.

\section{Open questions}
\label{sec:open}

The mechanization settles what the paper claimed; it also sharpens what
remains genuinely open. Nine questions, roughly in order of theoretical
depth:
...
Peritext by composition & \lean{peritextTF\_ra\_linearizable\_up\_to\_eq}, \lean{peritextRender\_congr} & \lean{MRDT\_Instances/PeritextTF/} \\
mark-anchor honesty composes & \lean{MarkAccurate}, \lean{peritextTF\_ra\_linearizable\_honest} & \lean{MRDT\_Instances/PeritextTF/MarkHonesty.lean} \\
