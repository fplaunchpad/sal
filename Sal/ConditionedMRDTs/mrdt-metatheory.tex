...
remains genuinely open. Eight questions, roughly in order of theoretical
depth:
...
$\mathsf{rc}$-edges but not $\vis$-separated ones. Moot for every current
conditioned instance, hence \lean{CausalCanonical} is a hypothesis with
two discharge lemmas rather than a theorem; a datatype needing both a
nontrivial $\mathsf{rc}$ and a version invariant (the budget cart,
\lean{BudgetCart}, of the instance roadmap) will force the question.
\end{openq}
