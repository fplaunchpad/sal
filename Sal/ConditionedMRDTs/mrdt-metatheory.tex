...
Peritext (tombstone-free, fused) & \emph{fused} & \multicolumn{2}{l}{\lean{peritextFused\_ra\_linearizable\_up\_to\_eq} --- one RGA at $\alpha := \mathsf{char} \oplus \mathsf{boundary}$; convergence by instantiation, genuine positional intent (\lean{render\_id\_active\_iff\_between}); 773 lines} \\
\bottomrule
\end{tabular}}
...
and it is now \textbf{built} (\lean{MRDT\_Instances/PeritextFused/}): rich text
...
only an independent spec does; and (ii) a residual that is \emph{not} a leak
--- under interior deletion the RGA re-sorts survivors
(\lean{del\_can\_reorder\_survivors}), a bounded reading-order change, the
atomicity horn traded for live positioning.
\end{openq}
...
Peritext, fused (positional intent) & \lean{peritextFused\_ra\_linearizable\_up\_to\_eq}, \lean{render\_id\_active\_iff\_between} & \lean{MRDT\_Instances/PeritextFused/} \\
