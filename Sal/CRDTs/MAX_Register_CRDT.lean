...
State is a single natural number; the only operation is `Write v`, which
updates the state to `max s v`. Merge is `max`. Because `max` is
commutative, associative, and idempotent, every RA-linearizability VC
