...
**Relationship to `in_span_boundary`.** The two predicates are not
literal refinements of each other. `in_span_boundary` captures the
four specific boundary positions with side-bit flips and is useful
for stating localized theorems (the expand/contract theorems above).
`in_span_visible` captures the whole span via visible-order and is
paper-faithful. There exist degenerate RGA states where a char `c`
with `afters(c) = startId` and `startSide = true` is traversed *past*
`endId` (e.g., when `c.opId < some sibling of startId's existing
children`); `in_span_boundary` would say such `c` is covered,
`in_span_visible` correctly says it isn't. In well-formed runs
generated by Peritext itself (where marks are applied to existing
text) such degeneracies don't arise, so `in_span_boundary` is
sound on generated states.

The existing expand/contract / priority-rule / partial-overlap
theorems are stated against `in_span_boundary`. A follow-up step
would restate them against `in_span_visible` to get paper fidelity;
that's left as separate work since it touches all the read-side
theorems. -/
