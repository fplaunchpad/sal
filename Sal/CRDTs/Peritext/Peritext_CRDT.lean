...
The paper's *interesting* merge semantics (expand/contract via anchor
sides, priority rule, coexistence of different mark types, etc.) are
characterized as theorems in a separate file
[`Peritext_ReadSide.lean`](Peritext_ReadSide.lean).  That file
imports this one and builds the read-side projection on top of the
spec defined here.  Splitting is for iteration speed: the 24 VCs
elaborate slowly and rarely change; the read-side theorems iterate
quickly once the VC file is cached.
