...
and `do_` distributes over `merge`. All 24 RA-linearizability VCs
follow as lattice facts. The paper's `distinct_ops` assumption ensures
that in practice every write has a unique timestamp; the value
tiebreak is there only to make `merge` and `do_` total on arbitrary
