import Sal.MRDTs.Metatheory.Development.Emulation.Labeled_TS

/-!
# Forwarding stub — content moved

The Labeled-TS core now lives in the metatheory development tree so it can be
modified there without touching the emulation framework's layout:
`Sal/MRDTs/Metatheory/Development/Emulation/Labeled_TS.lean`.
All declarations keep the `Sal.Emulation` namespace; this stub keeps every
existing `import Sal.Emulation.Labeled_TS` working unchanged.
-/
