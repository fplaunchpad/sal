...
\begin{abstract}
  ...
  bottom-up rules, and the both-empty / asymmetric /
  shared-last-event sub-cases of the existential induction are
  kernel-verified; three well-localised sorries remain, two of
  which are auxiliary lemmas (Lemma~1 of the Sal appendix:
  no $\mathit{lo}$-edge from $L^a$ to $L^b$) with their depth-1
  cases closed inline, and the third is the distinct-last-event
  branch awaiting a paper-faithful $L^a / L^b$ carving refactor.
  We report on the design
...
\begin{enumerate}[leftmargin=*]
  ...
  \item A mechanisation of \Sal{}'s bridge theorem for CRDTs, with
    ...
    machinery, seven bottom-up rules, and both-empty /
    asymmetric / shared-last-event sub-cases of the strong
    induction are kernel-verified, with three well-localised
    sorries remaining (Table~\ref{tab:sorries}).
...
the first lemma (paper sub-cases 1.a and 2.a.i) and the
vis-disjunct rule-out scaffold of the second lemma close inline; the
paper's depth-2 case-explosion (six sub-sub-cases on vis/rc flavours
of three $\mathit{lo}$-edges, lines 128--156) and the residual
rc-composition for the second lemma remain as \texttt{sorry}.

\paragraph{Live sorry inventory.}
For the merge case as a whole, three declarations carry
...
\begin{table}[h]
  ...
  \begin{tabular}{@{}llp{6.5cm}@{}}
    ...
    \midrule
    \texttt{Merge\_Linearization.lean:1620}
      & \texttt{no\_lo\_a\_to\_b}
      & Lemma~1(1) of the paper. Depth-1 sub-cases close inline;
        depth-2 case-explosion (six sub-sub-cases on vis/rc edges)
        consumes \texttt{vis\_trans} + \texttt{no\_rc\_chain} +
        $L_\top^a$ causal closure. \\
    \texttt{Merge\_Linearization.lean:1662}
      & \texttt{no\_lo\_top\_a\_to\_top\_b}
      & Lemma~1(2) of the paper. Vis-disjunct rule-out closes
        inline (via $L_\top$ causal closure); residual is the
        rc-composition $\mathit{lo}_{rc}\,e''\,e \wedge
        \mathit{lo}\,e\,e' \to \mathit{lo}\,e''\,e'$ at
        appendix.tex:158--178. \\
    \texttt{Merge\_Linearization.lean:1712}
      & \texttt{distinct\_last\_case}
      & Three internal sub-cases (paper Case~3b commute, Case~3a
        with $e_1 \in \ev_2$ shared, Case~3a with $e_2 \in \ev_1$
        shared) — exactly the cases the carving handles. To be
        subsumed by the carving-faithful refactor of the
        distinct-last-event branch (Block 6 of the
        \texttt{Sal/Emulation/PLAN.md} carving plan). \\
    \bottomrule
  \end{tabular}
...
\begin{itemize}[leftmargin=*]
  \item Block 6 of the carving plan (\texttt{Sal/Emulation/PLAN.md}):
    ...
    fresh-context multi-hour effort. Subsumes the three
    \texttt{distinct\_last\_case} sub-case sorries.
  \item Lemma~1 bodies (\texttt{no\_lo\_a\_to\_b} depth-2,
    \texttt{no\_lo\_top\_a\_to\_top\_b} rc-composition):
    each $\sim 100$ Lean lines, gated on either making
    $\mathit{vis}$-transitivity a derived structural fact or
    propagating it as a hypothesis on the bridge theorem. Open
    question on the implicit
    ``$\neg\mathit{commute}$ chain'' the paper invokes; may
    require a derivation or a $29$th VC.
\end{itemize}

Two architectural decisions accompanied this work. First, the
\texttt{SatisfiesVCs} structure grew from $25$ fields to $28$:
...
the distinct-last-event branch), and \texttt{merge\_peel\_comm}
(consumed by the convergence-based re-permutation when peeling
commuting events from the carving's frontier). All three are
discharged vacuously by Grow-Only Set and represent real
obligations only on CRDTs whose $\mathit{rc}$ is non-trivial.
...
Three sorries (Table~\ref{tab:sorries}) plus the simulation proof
...
convergence machinery, all seven bottom-up rules, and the
...
\begin{itemize}[leftmargin=*]
  \item \emph{Carving-faithful refactor of the distinct-last-event
    ...
    multi-hour effort. Subsumes the three
    \texttt{distinct\_last\_case} sub-case sorries.
  \item \emph{Lemma~1 bodies}
    (\texttt{no\_lo\_a\_to\_b} depth-2,
    \texttt{no\_lo\_top\_a\_to\_top\_b} rc-composition):
    each $\sim 100$ Lean lines. Gated on either deriving
    $\mathit{vis}$-transitivity as a structural fact about
    \texttt{Configuration} (sound but invasive across the
    \texttt{Step} rules) or threading it as a hypothesis on the
    bridge theorem. Open question on the implicit
    ``$\neg\mathit{commute}$ chain'' the paper invokes; may
    require a derivation or a $29$th VC. About $1$--$2$ weeks
    once the dependency strategy is decided.
