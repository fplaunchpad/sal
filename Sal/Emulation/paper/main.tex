...
\begin{enumerate}[leftmargin=*]
  ...
  \item Concrete effort estimates, derived from actual mechanisation
    experience rather than guesswork, for the open pieces
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}rlp{7.8cm}l@{}}
    ...
    1 & \texttt{RA\_Linearizability.lean} & $29$ VCs as a \texttt{SatisfiesVCs} struct & done \\
    2 & --- & bridge: base / \textsc{CreateReplica} / \textsc{Query} & done \\
    3 & --- & bridge: \textsc{Apply} case & done \\
    4 & \texttt{Merge\_Linearization.lean} & bridge: \textsc{Merge} case (existential, convergence + 7 bottom-up rules + both-empty / asymmetric / shared-last-event sub-cases + Lemma~1 closed; distinct-last-event Case~3b awaiting carving refactor) & partial \\
    ...
    \midrule
    \multicolumn{4}{l}{\footnotesize Live \texttt{sorry}s: $2$ declarations / $6$ internal occurrences, all in the \textsc{Merge} case (Table~\ref{tab:sorries}).}\\
    \bottomrule
  \end{tabular}
...
\begin{itemize}[leftmargin=*]
  \item Block 6 of the carving plan (\texttt{Sal/CRDTs/Metatheory/PLAN.md}):
...
\texttt{SatisfiesVCs} with $29$ fields. The first $24$
...
CRDT by pointing at the corresponding theorem. The remaining $5$
fields capture properties \Sal{}'s $24$ VCs leave implicit;
each emerged from a specific point in the mechanisation.
...
structure makes this implicit assumption explicit:
...
does not explicitly mechanise.

\paragraph{Fields $26$--$28$: lattice-bottom merge, directional
rc-non-comm, merge-peel-commute.} \texttt{merge\_init}
($\mmerge(\init, s) = s$) is consumed by the asymmetric
(one-side-empty) sub-case of \texttt{merge\_linearization\_exists};
$\Sal$'s file-level theorem set does not include it because the
asymmetric case is treated implicitly in the paper.
\texttt{rc\_non\_comm\_directional} ($\neg \commute(o_1, o_2)
\leftrightarrow \mrc(o_1, o_2) = \Fst \lor \mrc(o_2, o_1) = \Fst$)
is the directional variant of \texttt{rc\_non\_comm} that the
strictly-local sub-case of the distinct-last-event branch needs;
\Sal{}'s file-level statement is the symmetric $\Either$-shaped
form which doesn't yield a peel-direction witness when both events
are concurrent. \texttt{merge\_peel\_comm} (peel an event off the
left side of a merge when the right side's events all commute with
it) is consumed by the convergence-based re-permutation. All three
discharge vacuously for trivial-rc CRDTs.

\paragraph{Field $29$: \texttt{shared\_peel\_1op} (campaign-discovered).}
The Aristotle campaign (\S\ref{sec:aristotle-campaign}) identified
a CRDT-lattice property that \Sal{}'s $24$ VCs cannot derive: when
both replicas have applied the same LCA event $o_l$ and one has
additionally applied $o_1$, peeling $o_1$ out of the merge is sound:
\begin{lstlisting}[caption={The shared-element 1-op peel field.},
                   label=lst:shared-peel-1op]
shared_peel_1op :
...
\end{lstlisting}
The reason it is missing from \Sal{}'s base $24$: every existing
single-side-extension VC (\texttt{ind\_left\_*}, \texttt{ind\_right\_*},
\texttt{inter\_*}) requires \texttt{distinctOps} between the new
operation and the other side's tail. When both sides share $o_l$,
this requires \texttt{distinctOps o\_l o\_l}, which is always
false. The only ``shared LCA'' VC, \texttt{ind\_lca\_1op}, requires
both sides to start from the same base state, which doesn't hold
once each side has its own prefix. The shared peel is therefore a
genuinely new axiom; for typical CRDTs it follows from join
associativity / commutativity (e.g.\ for Grow-Only Set,
$(a \cup \{o_l\} \cup \{o_1\}) \cup (b \cup \{o_l\}) =
(a \cup \{o_l\} \cup b \cup \{o_l\}) \cup \{o_1\}$).
...
\begin{itemize}[leftmargin=*]
  \item \emph{Carving-faithful refactor of the distinct-last-event
    branch} (Block 6 of \texttt{Sal/CRDTs/Metatheory/PLAN.md}):
...
enough that downstream work (closing the residual sorries via
Block 6, extending to MRDTs, instantiating for the other $17$
CRDTs in the \Sal{} suite) can proceed without re-litigating
design choices. A $10$-run automated sorry-chain campaign with
Harmonic's \textsc{Aristotle} prover closed both halves of the
appendix's Lemma~1, decomposed the residual distinct-last-event
sorries into $6$ fine-grained sub-cases each pointing at the
same architectural fix (the $L^a / L^b$ carving), and identified
\texttt{shared\_peel\_1op} as a CRDT-lattice axiom missing from
\Sal{}'s base $24$ VCs.
