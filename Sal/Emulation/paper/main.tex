...
\begin{abstract}
  ...
  $24$ VCs of \Sal{} together with one additional semantic
  $\textsc{cond-comm}$ lift, the canonical op-based counterpart
  $\Emul^{-1}(\RDT)$ is also RA-linearisable on every reachable
  trace. The formalisation decomposes into $11$ steps; $9$ have
  complete or substantially complete Lean proofs, and the remaining
  two are the \emph{merge case} of \Sal{}'s bottom-up linearisation
  and the weak-simulation proof for the canonical emulation map.
  Within the merge case the convergence machinery, the seven
  bottom-up rules, and three of four sub-cases of the existential
  induction are kernel-verified; four well-localised sorries
  remain, all attributable to a closure-stability obstruction in
  the recursion that the Sal paper handles via its
  $L^a / L^b$ event-set decomposition. We report on the design
...
\begin{enumerate}[leftmargin=*]
  ...
  \item A mechanisation of \Sal{}'s bridge theorem for CRDTs, with
    ...
    machinery, seven bottom-up rules, and three of four
    sub-cases of the strong induction are kernel-verified, with
    four well-localised sorries remaining (Table~\ref{tab:sorries}).
  ...
  \item An end-to-end validation: for the Grow-Only Set we construct
    a \texttt{CRDTSig} instance and discharge all $25$ VC fields of
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}rlp{7.8cm}l@{}}
    ...
    4 & \texttt{Merge\_Linearization.lean} & bridge: \textsc{Merge} case (existential, convergence + 7 bottom-up rules + 3-of-4 sub-cases closed) & partial \\
    ...
    \midrule
    \multicolumn{4}{l}{\footnotesize Live \texttt{sorry}s: $4$, all in the \textsc{Merge} case (Table~\ref{tab:sorries}).}\\
    \bottomrule
  \end{tabular}
...
Two specific obstructions remain.

\paragraph{Recursion on shrunken event sets does not preserve
\texttt{differentReplicas}.}
At the top level of \texttt{merge\_linearization\_exists}, $\ev_1$
and $\ev_2$ equal specific replicas' event sets ($r_1$ and $r_2$
of $C$), so an event in $\ev_1 \setminus \ev_2$ is genuinely
``local to $r_1$'' and the
\texttt{vis\_causal} + \texttt{vis\_total\_same\_replica} chain
witnesses \texttt{differentReplicas} on any
$(\ev_1 \setminus \ev_2) \times (\ev_2 \setminus \ev_1)$ pair.
After one step of the shared-last-event recursion, $\ev_1$ and
$\ev_2$ have been shrunk by removing $e_1$; they no longer equal
any single replica's event set, and an event sitting in
``shrunken $\ev_1 \setminus \ev_2$'' may have entered there
because it was peeled off $\ev_2$ in a prior iteration, not
because it is local to $r_1$. The Sal paper's
$L^a / L^b$ decomposition (\S\ref{sec:merge-decomposition}) is
designed to be \emph{closure-stable} under exactly this recursion;
we have the decomposition definitions but have not yet refactored
the inductive argument to peel from $L_a$ rather than from the
list tail. The current sorry sits at
\texttt{Merge\_Linearization.lean:945}.

\paragraph{\texttt{merge\_init\_left\_reachable} for $|\pi| \geq 2$.}
The asymmetric case requires
$\mmerge(\init, \mathit{applySeq}(\init, \pi)) = \mathit{applySeq}(\init, \pi)$
for general reachable $\pi$. The base cases $|\pi| \in \{0, 1\}$
close via \texttt{merge\_idem}, \texttt{merge\_comm},
\texttt{base\_1op}; for $|\pi| \geq 2$ no $\textsc{Ind-*-1op}$ /
$\textsc{Inter-*-1op}$ rule extends the singleton case beyond the
$b = \init$ line, because every such VC requires the right side of
$\mmerge$ to have already received an event $o_l$. The Sal paper
closes this case as a \emph{byproduct} of the same outer
induction it uses for the merge witness; we expect to follow that
path once the closure-stable carving lands. Sorry at
\texttt{Merge\_Linearization.lean:743}; a related placeholder for
``signature tightening to $\ev = C.\mathit{events}$'' sits at
\texttt{:474} inside the convergence call site.

\paragraph{Live sorry inventory.}
For the merge case as a whole, the live sorries are exactly the
following (no others remain in the bridge or emulation
...
\begin{table}[h]
  ...
  \begin{tabular}{@{}llp{6.5cm}@{}}
    \toprule
    File:line & Theorem & What's open \\
    \midrule
    \texttt{RA\_Linearizability.lean:652}
      & \texttt{RA\_lin\_preserved\_merge}
      & Shim that dispatches to
        \texttt{Merge\_Linearization}. Closes with a one-line call to
        \texttt{RA\_lin\_preserved\_merge\_via\_witness} once the
        existential is closed. \\
    \texttt{Merge\_Linearization.lean:474}
      & inside \texttt{convergence}
      & Overwriter-witness obligation at the bubble-sort call site;
        resolved by tightening \texttt{convergence}'s signature to
        $\ev = C.\mathit{events}$. \\
    \texttt{Merge\_Linearization.lean:743}
      & \texttt{merge\_init\_left\_reachable}
      & $|\pi| \geq 2$ inductive step (no VC extends
        $\mmerge\,X\,\init$ beyond the singleton case). \\
    \texttt{Merge\_Linearization.lean:945}
      & distinct-last-event branch of
        \texttt{merge\_linearization\_exists}
      & \texttt{differentReplicas} does not survive recursion on
        shrunken $\ev_1 \setminus \ev_2$; needs the
        $L^a / L^b$ closure-stable carving. \\
    \bottomrule
  \end{tabular}
  \caption{Live sorries in the bridge theorem.}
  \label{tab:sorries}
\end{table}
...
all seven bottom-up rules, the existential's both-empty,
asymmetric (modulo \texttt{merge\_init\_left\_reachable}), and
shared-last-event branches are closed. Realistic remaining effort:
\begin{itemize}[leftmargin=*]
  \item \texttt{Merge\_Linearization.lean:474} (overwriter witness):
    $1$--$2$ days, mostly signature plumbing.
  \item \texttt{merge\_init\_left\_reachable} ($|\pi| \geq 2$):
    $1$--$2$ weeks, coupled to the $L^a / L^b$ refactor.
  \item Distinct-last-event closure-stable refactor:
    $1$--$2$ weeks, the bulk of which is the
    $L^a / L^b$ carving and propagating it through the
    induction.
\end{itemize}

The packaging theorem
\texttt{RA\_lin\_preserved\_merge\_via\_witness} (in
\texttt{Merge\_Linearization.lean}) destructures the existential
and is closed unconditionally; once
\texttt{merge\_linearization\_exists} closes, swapping the stubbed
\texttt{RA\_lin\_preserved\_merge} in
\texttt{RA\_Linearizability.lean} for a one-line call to
\texttt{RA\_lin\_preserved\_merge\_via\_witness} finishes the
bridge theorem.
...
Four sorries (Table~\ref{tab:sorries}) plus the simulation proof
for $\Emul$ remain open. We report estimates based on actual
mechanisation experience with the closed pieces, including the
substantial portion of the merge case that has now landed: the
convergence machinery, all seven bottom-up rules, and three of
the four sub-cases of \texttt{merge\_linearization\_exists}.

\begin{itemize}[leftmargin=*]
  \item \emph{Overwriter-witness sorry inside
    \texttt{convergence}} (\texttt{Merge\_Linearization.lean:474}):
    resolved by tightening \texttt{convergence}'s signature to
    $\ev = C.\mathit{events}$ and propagating an
    overwriter-closure invariant through the bubble-sort. Mostly
    plumbing. About $1$--$2$ days.
  \item \emph{\texttt{merge\_init\_left\_reachable} for
    $|\pi| \geq 2$} (\texttt{:743}): needs the closure-stable
    $L^a / L^b$ carving (next item) since the $24$ VCs as
    standalone rules do not cover $\mmerge\,X\,\init$ beyond the
    singleton case. Likely closes as a byproduct once the carving
    lands. About $1$--$2$ weeks, coupled to the next item.
  \item \emph{Distinct-last-event branch of
    \texttt{merge\_linearization\_exists}} (\texttt{:945}):
    refactor the existential's induction to peel from $L_a$ rather
    than from the list tail, threading the
    $L^a / L^b$ partition through the recursion to preserve
    closure of \texttt{differentReplicas}. About $1$--$2$ weeks.
    This and the previous item share infrastructure and will likely
    close together.
  \item \emph{\texttt{effectiveState} linear extension and weak
    ...
    $2$--$4$ weeks; the second-longest remaining item after the
    distinct-last-event refactor.
\end{itemize}
