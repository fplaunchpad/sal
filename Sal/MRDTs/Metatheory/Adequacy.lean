...
**per-version** RA-linearizability (`IsRALinearizableJoin`, the paper's Def-lin
...
replica-keyed core. -/
def IsRALinearizableJoin (C : Configuration D) : Prop :=
...
theorem isRALinearizable3_of_good {C : Configuration D}
    (h : GoodConfig3 C) : IsRALinearizableJoin C :=
...
theorem goodConfig3_init : GoodConfig3 (initConfig D) := by
  ...
  · -- canonical
    ...
    · rw [hwn, hver_new, Option.some.injEq, Prod.mk.injEq] at hw
      ...
      have h_old' : IsCanonicalState (Configuration.core C') ev s := by
        ...
        rw [Configuration.core_vis, Configuration.core_vis]
        exact h_vis_old ev h_ev_events a ha b hb
      have h_ext := isCanonicalState_extend (e := e) he_not_ev
        (fun x hx => by
          rw [Configuration.core_vis, hvis]; exact Or.inr ⟨hx, rfl⟩)
        (fun x hx => by
          rw [Configuration.core_vis, hvis]
          rintro (hex | ⟨he_ev, _⟩)
