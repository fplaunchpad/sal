import Sal.MRDTs.Metatheory.Development.RGA_BirthBridge_Split
...
namespace Sal.Metatheory.RGABirthBridgeHRc
...
open Sal.Metatheory.RGABirthBridgeSplit (split_liveChain)
...
theorem hRc_bii (σ₀' σ₁' σ₂' : concrete_st) (F : List op_t) (t r e a : ℕ) (p : List ℕ)
  ...
  · intro c hc
    exact hpreDead c (by rw [hsp]; exact List.mem_append.mpr (Or.inl hc)) (hpd c hc)
  · intro hcbw
    exact hFiltRecon (anc σ₁' t) rcSuf hcbw hlive
...
theorem hFiltRecon_of_canonInv (σ₀' σ₁' : concrete_st) (F ρ₀ : List op_t)
    ...
    ∀ (w : ℕ) (rc : List ℕ),
        contains σ₀' w = true → IsAncPath σ₁' w (liveSub σ₁' rc) →
  ...
  rw [canonAnc_filter_surv F (liveSub σ₀' (a_w :: p_w)), canonAnc_filter_surv F rc,
    hFiltEq w (liveSub σ₀' (a_w :: p_w)) rc hpath hp1]

#print axioms hFiltRecon_of_canonInv

end Sal.Metatheory.RGABirthBridgeHRc
