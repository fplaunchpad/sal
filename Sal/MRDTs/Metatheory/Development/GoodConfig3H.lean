import Sal.MRDTs.Metatheory.Development.GenericEqQuotient_H
import Sal.MRDTs.Metatheory.Development.GoodConfig3NF
...
namespace Sal.Metatheory.GoodConfig3H

open Sal.Emulation
open Sal.Metatheory.GenericEqQuotient
open Sal.Metatheory (Configuration Version Step3 Label3 IsLCA initConfig labeledTS3 core_vis)
...
theorem GoodConfig3S.ofGood {C : Configuration D'} (h : Sal.Metatheory.GoodConfig3 C) :
    GoodConfig3S C :=
...
def GoodConfig3H (E : EqEquiv D) (W : Op D.AppOp → D.State → Prop)
  ...
  ∀ (v : Version) (s : QState D E) (Ev : Set (Op D.AppOp)),
    C.ver v = some (s, Ev) →
    IsCanonicalStateH H E W hP hC hA (Sal.Metatheory.Configuration.core C) Ev s
    ...
    ∃ (σ : D.State) (hσ : D.Inv σ),
      ...
      ∃ π : List (Op D.AppOp),
        ...
        respects π (Sal.Emulation.lo (D := (QSig E W hP hC hA).toCRDTSig)
          (Sal.Metatheory.Configuration.core (D := QSig E W hP hC hA) C)) ∧
...
theorem isRALinearizable3Eq_of_goodH (E : EqEquiv D) (W : Op D.AppOp → D.State → Prop)
  ...
  have h1 : respects ρ (Sal.Emulation.loOn (D := (QSig E W hP hC hA).toCRDTSig)
      (Sal.Metatheory.Configuration.core (D := QSig E W hP hC hA) C) Ev) :=
    (respects_congr (loOn_qsig_iff E W hP hC hA
      (Sal.Metatheory.Configuration.core (D := QSig E W hP hC hA) C) Ev)).mpr hresp
  exact respects_lo_of_respects_loOn
    (C := Sal.Metatheory.Configuration.core (D := QSig E W hP hC hA) C) h1
...
theorem goodConfig3H_init (E : EqEquiv D) (W : Op D.AppOp → D.State → Prop)
  ...
  refine ⟨GoodConfig3S.ofGood (Sal.Metatheory.goodConfig3_init trivial), ?_⟩
  intro v s' E' hv
...
end Sal.Metatheory.GoodConfig3H
