import Sal.MRDTs.Metatheory.Development.UpdateFeasibility_Gate
...
namespace Sal.Metatheory.ConditionedConvergence
...
`Sal.Metatheory.G2AppAware.appliesDependsOn` (`e₂` names `e₁`'s Ins-timestamp).
...
`e₁`.  The direct generalization of `Sal.Metatheory.G2AppAware.loOnA`. -/
...
1. **Order VC `dependency_covers_vacuity` — DISCHARGEABLE.**  The witnessing
   ...
   `init_st` but applicable after `insOpE`.  For a full RGA discharge one shows:
   any vis-edge `a → b` with `commutesOn a b ∧ ¬commutes a b` is a
...
/-- **RGA discharge of the order VC on the counterexample pair.**
`appliesDependsOn RGACondSig delOpE insOpE` holds: `delOpE` is inapplicable at
`init_st` (node 1 absent) but applicable after `insOpE` (node 1 present), so
`insOpE` changes `delOpE`'s applicability.  This is the generic-`appliesDependsOn`
witness that `dependency_covers_vacuity` would consume for the `insOpE → delOpE`
edge that `no_loOnC_edge` shows conditioning drops. -/
theorem rga_appliesDependsOn_del_ins : appliesDependsOn RGACondSig delOpE insOpE := by
  refine ⟨init_st, fun h => ?_⟩
  have hnotP : ¬ RGACondSig.applicable delOpE init_st :=
    Sal.Metatheory.UpdateFeasibilityGate.delNode1_not_applicable 2 0 init_st (by simp [init_st])
  have hQ : RGACondSig.applicable delOpE (do_ init_st insOpE) :=
    Sal.Metatheory.G2Probe.delOpE_applicable_after_ins
  exact hnotP (cast h.symm hQ)
...
#print axioms rga_appliesDependsOn_del_ins

end Sal.Metatheory.ConditionedConvergence
