...
\item \textbf{In progress.} Theorem~\ref{thm:conv} itself: with
  Lemma~\ref{lem:swap} and Prop.~\ref{prop:thread} both in hand, it is the
  ordinary bottom-up canonicalization --- a proven template for flat types ---
  instantiated with the faithful swap at each prefix fold. Its assembly (and a
  routine generation base case) is the remaining mechanization; no open
  mathematical question is known to stand in its way.
