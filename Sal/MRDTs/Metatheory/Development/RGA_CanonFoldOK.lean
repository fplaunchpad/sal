import Sal.MRDTs.Metatheory.Development.RGA_CanonConvergence
...
conditional on `CanonFoldOK [] init_st π` — the per-event discipline at each
...
(§6) — two `loOnA`-respecting enumerations of `E` fold from `init_st` to
...
namespace Sal.Metatheory.RGACanonFoldOK
...
def DepE (Cfg : Sal.Emulation.Configuration RGACondSig.toCRDTSig)
    (E : Set op_t) (z x : op_t) : Prop :=
  z ∈ E ∧ loOnA RGACondSig Cfg E z x
...
def DepC (Cfg : Sal.Emulation.Configuration RGACondSig.toCRDTSig)
...
def IsDepPreC (Cfg : Sal.Emulation.Configuration RGACondSig.toCRDTSig)
...
def GenDisc2C (Cfg : Sal.Emulation.Configuration RGACondSig.toCRDTSig)
    (E : Set op_t) : Prop :=
  ∀ o ∈ E, ∀ d : List op_t, IsDepPreC Cfg E o d →
...
def GoodEnum (Cfg : Sal.Emulation.Configuration RGACondSig.toCRDTSig)
    (E : Set op_t) (σ : List op_t) : Prop :=
  (∀ x ∈ σ, x ∈ E) ∧ σ.Nodup ∧ respects σ (loOnA RGACondSig Cfg E) ∧
  (∀ x ∈ σ, ∀ z ∈ E, z ≠ x → loOnA RGACondSig Cfg E z x → z ∈ σ)

/-! ## §2  Fold-domain groundwork: ids enter only by their own `Ins` -/

/-- A single step adds no id but an `Ins`'s own. -/
theorem contains_do_cases (s : concrete_st) (o : op_t) (c : ℕ)
...
theorem contains_init (c : ℕ) : contains init_st c = false := by
  simp [init_st]

/-- An id live at a fold from `init_st` was inserted in the fold. -/
theorem insertedIn_of_contains_fold (L : List op_t) (c : ℕ)
    (h : contains (applySeqR init_st L) c = true) : insertedIn L c := by
...
theorem deletedIn_mono {L₁ L₂ : List op_t} (h : ∀ x ∈ L₁, x ∈ L₂) {c : ℕ}
...
theorem accurate_ins_entries (t r e a : ℕ) (p : List ℕ) (s : concrete_st)
...
theorem goodEnum_append (Cfg : Sal.Emulation.Configuration RGACondSig.toCRDTSig)
    (E : Set op_t) (F : List op_t) (o : op_t)
    (h : GoodEnum Cfg E (F ++ [o])) :
    GoodEnum Cfg E F ∧ o ∈ E ∧ o ∉ F ∧
      (∀ x ∈ F, ¬ loOnA RGACondSig Cfg E o x) := by
  ...
  have hcross : ∀ a ∈ F, ∀ b ∈ [o], ¬ loOnA RGACondSig Cfg E b a :=
    (List.pairwise_append.mp hresp).2.2
  have hlast : ∀ x ∈ F, ¬ loOnA RGACondSig Cfg E o x :=
...
theorem mem_of_depC (Cfg : Sal.Emulation.Configuration RGACondSig.toCRDTSig)
    (E : Set op_t) (σ : List op_t) (hg : GoodEnum Cfg E σ) :
    ∀ (x z : op_t), DepC Cfg E z x → x ∈ σ → z ∈ σ := by
  intro x z h
  induction h with
  | @single b h' =>
    intro hb
...
noncomputable def depList (Cfg : Sal.Emulation.Configuration RGACondSig.toCRDTSig)
    (E : Set op_t) (F : List op_t) (w : op_t) : List op_t :=
  F.filter (fun z => decide (z ≠ w ∧ DepC Cfg E z w))

theorem mem_depList {Cfg : Sal.Emulation.Configuration RGACondSig.toCRDTSig}
    {E : Set op_t} {F : List op_t} {w x : op_t} :
...
theorem isDepPreC_depList (Cfg : Sal.Emulation.Configuration RGACondSig.toCRDTSig)
    (E : Set op_t) (F : List op_t) (w : op_t)
    (hmemE : ∀ x ∈ F, x ∈ E) (hnd : F.Nodup)
    (hresp : respects F (loOnA RGACondSig Cfg E))
...
theorem goodEnum_depList_last (Cfg : Sal.Emulation.Configuration RGACondSig.toCRDTSig)
    (E : Set op_t) (F : List op_t) (o : op_t)
...
theorem isDepPreC_depList_last (Cfg : Sal.Emulation.Configuration RGACondSig.toCRDTSig)
    (E : Set op_t) (F : List op_t) (o : op_t)
    (hg : GoodEnum Cfg E (F ++ [o])) :
...
theorem isDepPreC_depList_mem (Cfg : Sal.Emulation.Configuration RGACondSig.toCRDTSig)
    (E : Set op_t) (F : List op_t) (w : op_t)
...
theorem resolve_restrict (F d : List op_t)
    (hdsubF : ∀ x ∈ d, x ∈ F)
    (h0F : contains (applySeqR init_st F) 0 = false)
    (hdomF : ∀ c, contains (applySeqR init_st F) c = true ↔ survP F c)
    (hdomD : ∀ c, contains (applySeqR init_st d) c = true ↔ survP d c) :
    ∀ L : List ℕ, (∀ c ∈ L, c = 0 ∨ insertedIn d c) →
      resolve (applySeqR init_st F) L
        = resolve (applySeqR init_st F) (liveSub (applySeqR init_st d) L) := by
  intro L
  induction L with
  | nil => intro _; rfl
  | cons c cs ih =>
    ...
    set sF := applySeqR init_st F
    set sD := applySeqR init_st d
...
theorem anc_transport (F d : List op_t)
    (hdsubF : ∀ x ∈ d, x ∈ F)
    (hinvF : CanonInv F (applySeqR init_st F))
    (hinvD : CanonInv d (applySeqR init_st d))
    (hchains : ∀ z rz ez az (pz : List ℕ), (z, rz, .Ins ez pz az) ∈ d →
        ∀ c ∈ az :: pz, c = 0 ∨ insertedIn d c)
    (z : ℕ) (hzD : contains (applySeqR init_st d) z = true)
    (hzF : contains (applySeqR init_st F) z = true)
    (W : List ℕ) (hW : IsAncPath (applySeqR init_st d) z W) :
    anc (applySeqR init_st F) z = resolve (applySeqR init_st F) W := by
...
theorem chainOK_transport (F d : List op_t)
    (hdsubF : ∀ x ∈ d, x ∈ F)
    (hinvF : CanonInv F (applySeqR init_st F))
    (hinvD : CanonInv d (applySeqR init_st d))
    (hchains : ∀ z rz ez az (pz : List ℕ), (z, rz, .Ins ez pz az) ∈ d →
        ∀ c ∈ az :: pz, c = 0 ∨ insertedIn d c) :
    ∀ (W : List ℕ) (z : ℕ),
      contains (applySeqR init_st d) z = true →
      IsAncPath (applySeqR init_st d) z W →
      ∀ c cs, liveSub (applySeqR init_st F) (z :: W) = c :: cs →
        IsAncPath (applySeqR init_st F) c cs := by
  intro W
  induction W with
  | nil =>
...
theorem canonFoldOK_append :
    ∀ (π F : List op_t) (s : concrete_st) (o : op_t),
...
theorem chain_entries_mem (Cfg : Sal.Emulation.Configuration RGACondSig.toCRDTSig)
    (E : Set op_t) (hGen : GenDisc2C Cfg E) (F : List op_t)
    (hgF : GoodEnum Cfg E F) (t' r' e' a' : ℕ) (p' : List ℕ)
    (hm : (t', r', .Ins e' p' a') ∈ F) :
    ∀ c ∈ a' :: p',
...
theorem depPack_last (Cfg : Sal.Emulation.Configuration RGACondSig.toCRDTSig)
    (E : Set op_t) (hGen : GenDisc2C Cfg E)
    (F : List op_t) (o : op_t) (hg : GoodEnum Cfg E (F ++ [o]))
    (hIH : ∀ σ : List op_t, σ.length ≤ F.length → GoodEnum Cfg E σ →
...
theorem canonStepOK_of_gen (Cfg : Sal.Emulation.Configuration RGACondSig.toCRDTSig)
    (E : Set op_t)
...
theorem canonFoldOK_of_gen (Cfg : Sal.Emulation.Configuration RGACondSig.toCRDTSig)
    (E : Set op_t)
    (hdts : ∀ a b : op_t, a ∈ E → b ∈ E → a ≠ b → a.1 ≠ b.1)
    (hids0 : ∀ o ∈ E, o.1 ≠ 0)
    (hGen : GenDisc2C Cfg E) :
    ∀ (n : ℕ) (σ : List op_t), σ.length ≤ n → GoodEnum Cfg E σ →
...
end Sal.Metatheory.RGACanonFoldOK
