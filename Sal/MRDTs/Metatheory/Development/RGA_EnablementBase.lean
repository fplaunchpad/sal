import Sal.MRDTs.Metatheory.Development.RGA_FaithfulThreading_Gate
import Sal.MRDTs.Metatheory.Development.RGA_ConditionedConvergence
...
open Sal.Metatheory.RGAGeneralSwap (Faithful)
open Sal.Metatheory.RGAConditionedConvergence (chainFaithful_of_accurate)
...
`Sal.Metatheory.RGAStaledDelGate.sIns` has the two live root-children `{2,8}`; its
...
open Sal.Metatheory.RGAStaledDelGate (sIns Lclash contains_sIns)
