...
no \texttt{sorryAx}/\texttt{native\_decide}/\texttt{ofReduceBool}. Status as of this writing.
...
\hline
Two-sided merge $\Rightarrow$ \lean{CanonBirthBridge} & \lean{eq\_merge\_two\_sided\_of\_foldChain}
  & proved (mod.\ $\mathsf{hin}$)\\
$\mathsf{hin}$ (branch-canonical) $\to$ close merge & \lean{RGA\_BranchCanon} & in mechanization\\
$\mathsf{WfOpReachable}$ for the RGA & \lean{RGA\_WfOpReachable} & in mechanization\\
$\mathsf{EqJoinLemma3C}$ assembly $+$ bundle $\to$ RGA RA-lin & --- & awaits the two above\\
Flat MRDTs (MVR, AW-PQ) via identity quotient & --- & after the framework\\
\hline
\end{tabular}
\end{center}

\paragraph{Reading.} The two genuinely hard problems --- convergence of the tombstone-free RGA
under rehoming, and making the metatheorem generic over $\eqobs$ --- are both \emph{proved and
kernel-clean}: the first by canonical-state characterization (after the swap route was refuted
twice), the second by the $\eqobs$-quotient functor \lean{RA\_linearizable\_up\_to\_eq}. No open
mathematical question remains and no semantic distortion remains (the theorem is over the real
$\dodo$). The residual is bounded assembly over verified lemmas: discharge $\mathsf{WfOpReachable}$
(from monotone ids and the recorded-path structure), close the merge $\mathsf{hin}$ (a
branch-canonical induction, the update technique re-run single-sided), assemble $\mathsf{EqJoinLemma3C}$
from Lemma~\ref{lem:canon} plus the merge bridge, and bundle the five VCs into
$\mathsf{RA\_linearizable\_up\_to\_eq}$ to obtain the RGA instance. Property-based testing
corroborates every step (600 merge-vs-fold triples through the hard corners including
cross-branch anchor and shared delete, zero counterexamples). The flat MRDTs then discharge the
same five VCs trivially ($\eqobs$ is $=$, the quotient is the identity) and inherit
RA-linearizability from the same theorem.
\end{document}
