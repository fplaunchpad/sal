...
Generic Join $\Rightarrow$ RA-lin (Inv-conditioned, structural $=$) &
  \lean{ra\_linearizable3\_of\_joinC} & proved (template)\\
$\eqobs$-quotient state layer (hosts the RGA on the $=$-bridge) & (M5; no quotient exists yet) &
  to build\\
$\app$-conditioning on top of $\Inv$ & (extends the template) & to build\\
\hline
\end{tabular}
\end{center}
...
template, and --- verified against \lean{Adequacy.lean} --- neither is free transcription. (i)
The template is stated over \emph{structural} equality ($\lean{IsRALinearizable3}$ demands
$\fold\,\ini\,\pi = s$, and its Join Lemma is $s = \merge\,l\,t\,(\text{update})$), whereas the
RGA converges only up to $\eqobs$. Reconciling these is a real construction, and precisely the
deferred M5 layer: either re-prove the bridge over an $\eqobs$-congruence (a substantial
re-proof of a currently kernel-clean file), or --- cleaner, and what we adopt --- present the
RGA state as a quotient by $\eqobs$, so structural $=$ on the quotient \emph{is} $\eqobs$ and
the existing $=$-bridge hosts the RGA unchanged; the cost is lifting $\dodo$ and $\merge$ to the
quotient, which they permit since they are $\eqobs$-congruent
(\lean{do\_eq\_congr}, \lean{applySeqR\_eq\_congr}). No such quotient layer exists yet. (ii)
Adding the applicability conditioning $\app$ on top of $\Inv$ extends the template's premises.
The two lemmas of Theorem~\ref{thm:meta} --- the convergence engine and the adequacy induction
--- are then reused: the engine already over $\eqobs$, the adequacy bridge over the quotient. On the RGA side, the
...
question remains. The residual is genuine but bounded: transcription of $\RVF$; the
$\eqobs$-quotient state layer (M5) and the $\app$ premise-extension of the template --- real
constructions, not transcription, as the check against \lean{Adequacy.lean} above shows; and
the mechanical assembly they unlock.
\end{document}
