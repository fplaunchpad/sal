...
(the conditioned framework and the RGA chain live under
\lean{Conditioned/}, the investigation record under \lean{Development/};
each cited theorem kernel-checked).
...
once. This ``read the shared past off the LCA'' pattern is what
separates MRDTs from CRDTs, and \S\ref{sec:loadbearing} proves the
separation is real.
...
\item $\Inv$: well-formed forest (every stored anchor live or root),
  id-monotone anchors ($\anc(x) < x$), root unstored. $\app$:
  \emph{accurate} (the recorded path is the target's true live ancestor
  chain in the current state) and \emph{fresh} (the id is unused).
  $\eqobs$: equal live forests --- same nodes, elements and anchors.
\end{itemize}
A three-node example shows why every piece is needed. Start from the
chain $0 \leftarrow 1 \leftarrow 2$ (node $1$ under the root, node $2$
...
file \lean{RGA\_TombstoneFree\_RA\_Lin.lean}) closes
...
at the pair's common causal base. That architecture now exists, and \S\ref{sec:condmeta}--\S\ref{sec:actual}
of this note develop it in full: it is not a repaired bubble --- the swap
frame itself is refuted at merge unions --- but a conditioned metatheory
whose target is the raw fold up to $\eqobs$. The ladder above is the
sequence of kernel-checked refutations that found it; the resolution is
the end-to-end theorem \lean{rga\_tombstone\_free\_ra\_linearizable3\_eq}.
\end{openq}
...
peel obstruction (OQ~\ref{oq:reunify}) & \lean{reunification\_peel\_obstruction}, \lean{no\_proper\_back\_block} & \lean{Conditioned/\{Reunification\_Peel\_Obstruction,JoinLemma3C\}.lean} \\
...
closure-indexed adequacy + instances & \lean{ra\_linearizable3\_of\_joinC}, \lean{ConditionedContract} & \lean{Conditioned/ConditionedContract.lean} \\
all 9 discharges via the contract & \lean{$\ast$\_adequate\_viaContract} (9) & \lean{Conditioned/AllMRDTs\_viaContract.lean} \\
update-layer feasibility notion & \lean{loOnA\_noopFeasible\_verdict} & \lean{Conditioned/UpdateFeasibility\_Gate.lean} \\
conditioned convergence (order-half) & \lean{conditioned\_convergence\_on} & \lean{Conditioned/ConditionedConvergence.lean} \\
...
feasible-update gate & \lean{G2\_conditioned\_convergence\_refuted}, \lean{separating\_inequivalence} & \lean{Conditioned/G2\_\{Transport\_Probe,Applicability\_Aware\}.lean} \\
\midrule
...
observational quotient $D \mapsto D_\approx$ & \lean{QSig}, \lean{qapplicable} & \lean{Conditioned/GenericEqQuotient$\ast$.lean} \\
canonical-witness layer, raw-$\approx$ target & \lean{IsRALinearizable3Eq}, \lean{RA\_linearizable\_up\_to\_eq\_H} & \lean{Conditioned/GoodConfig3H.lean} \\
RGA canonical engine & \lean{CanonInv}, \lean{canon\_fold} & \lean{Conditioned/RGA\_Canon$\ast$.lean} \\
generation discipline from born accuracy & \lean{genDisc2C\_of\_born} & \lean{Conditioned/RGA\_GenDisc\_Assembly.lean} \\
capstone + discharged leaves & \lean{rga\_RA\_linearizable\_skeleton3} & \lean{Conditioned/RGA\_Skeleton3.lean}, \lean{RGA\_$\ast$\_Discharge.lean} \\
honest residual & \lean{HonestDelivery}, \lean{rga\_RA\_linearizable\_honest} & \lean{Conditioned/RGA\_Honest\_Residual.lean} \\
...
The conditioned framework and the full RGA chain live under
\lean{Conditioned/} (importable from the mainline, which never imports
\lean{Development/}); the refutation probes, findings journals and
superseded routes remain under \lean{Development/} as the research
record. Each cited theorem is kernel-checked
