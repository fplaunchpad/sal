...
import Sal.MRDTs.Metatheory.Conditioned.RGA_Instance_Final
...
(`BornApplicable_Guard`); `CongVC` is guard-independent (`rgaCongVC'`); the
...
open Sal.Metatheory.RGAInstanceFinal (rgaInvInvVCQ)

/-- **`InvInvVC RGACondSig' rgaEqEquiv' WfOpA`.**  `wf_congr`: `WfOpA = WfOpQ ∧
...
def rgaInvInvVCA : InvInvVC RGACondSig' rgaEqEquiv' WfOpA where
  wf_congr := by
    intro o s s' Is Is' h
