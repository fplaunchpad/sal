...
open Sal.Metatheory.RGACanonFoldOK (canonFoldOK_append insertedIn_of_contains_fold)

/-- **hHext, discharged.**  The witness discipline `rgaH` extends at an applicable apply. -/
theorem rga_hHext_discharged
    {C₀ C₁ : Sal.Metatheory.Configuration
  ...
  have hstate : applySeq RGACondSig'.toCRDTSig RGACondSig'.init ρ = applySeqR init_st ρ :=
    Sal.Metatheory.RGAInstanceFinal.applySeq_eq_applySeqR RGACondSig'.init ρ
  rw [hstate] at happ
