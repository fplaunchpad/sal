...
def shared : QState :=
  applySeq Q.toUpdateSig Q.init [enqA, enqB]
...
example :
    (applySeq Q.toUpdateSig Q.init [enqA, enqB, deqA, deqB]).map Prod.snd = [20] := by
  decide

example :
    (applySeq Q.toUpdateSig Q.init [enqA, enqB, deqB, deqA]).map Prod.snd = [20] := by
  decide
