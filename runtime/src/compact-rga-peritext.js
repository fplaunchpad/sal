...
function cutFromMeet(meet) {
  const settledIds = new Set(), settledDelIds = new Set();
  for (const op of meet.values()) {
    for (const p of Array.isArray(op.payload) ? op.payload : op.payload ? [op.payload] : []) {
...
function compact(s, cut = {}) {
  ...
  const state = Object.freeze({ text: { shadow, deleted: dt.freeze() }, marks: s.marks });
