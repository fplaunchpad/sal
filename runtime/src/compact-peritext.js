// MARKS-LAYER STATE GC for Peritext: the retention-roots compaction plus the
// A3 guarded pair-drop, over the marks layer.
//
// THE KEEP-SET. Peritext keeps text deletes LOGICAL (a shadow of every birth
// plus a grow-only deleted set) because the mark resolver rehomes a DEAD
// boundary anchor to its nearest surviving neighbour in reading order, which
// needs the dead anchor's birth position. Pruning a dead anchor's record flips
// reads, so plain state compaction is unsound for peritext (the noRetention
// negative control, opts.noRetention, keeps that failure executable). The sound
// compaction keeps
//
...
// cut.frozenAnchorCoords, exact in every epoch).
//
// A3 GUARDED PAIR-DROP: an (add m, remove r) pair with equal boundary tuples
// (ids AND sides), same mtype, r.mid > m.mid resolves identically forever, and
// r beats m wherever both apply, so the pair is render-neutral and droppable
// (freeing its retention roots) ONLY under three guards:
//   (i)   the removal is settled at the cut (else an in-flight same-mtype add
//         between the mids is resurrected by the drop); declared in-flight
//         marks of the mtype predating r also refuse;
...
//         among present-or-declared ids (such a char is grabbed by the add's
//         end-growth but not the remove's). Future mints are Lamport-fresh
//         (> r.mid), so the check at the cut is sufficient.
//
...
//                    in-flight marks,
// }
// opts = { pairDrop: false            -- disable A3 (keep-set only),
//          unguardedPairDrop: true    -- NEGATIVE CONTROL ONLY,
//          noRetention: true          -- NEGATIVE CONTROL ONLY,
...
// shadow through the run-table serializer instead, plus an ID SIDECAR: record
// ids in the serializer's deterministic element order (entries in table order,
// members in order), zigzag-varint gap-coded. The sidecar is required because
// ids are NOT recoverable as delta sums after a compaction epoch (rank
// renumbering forgets time), and the marks/deleted layers anchor on real ids.
// The fingerprint is untouched, so compaction-commit content ids do not move;
// only the stored/wire snapshot bytes change.
...
/** A3 pair enumeration. A pair is (add m, remove r): same mtype, same boundary
 *  ids AND sides, r.mid > m.mid. With `unguarded` (negative control) every
 *  syntactic pair is returned; otherwise the three guards above all apply. */
...
function compactPeritextWith(state, cut, opts, textCompact, coordOfRecord, recordCost) {
  ...
  const noRetention = opts.noRetention === true;             // NEGATIVE CONTROL
  const unguardedPairDrop = opts.unguardedPairDrop === true; // NEGATIVE CONTROL
  ...
  // --- the keep-set: live UNION mark boundary anchors (surviving records AND
  // declared in-flight marks) UNION declared in-flight insert anchors. Under
  // noRetention only the declared anchors are kept.
  ...
  // --- drop rule: a record vanishes iff its insert AND its delete are settled
  // at the cut and no retention root claims it. Live and unsettled records are
  // always kept.
  ...
  // anchor's CURRENT-epoch coordinate ('' = root).
 ...
 *  compacts this object (with the certificate) via the marks-layer GC.
...
export const compactiblePeritext = {
  ...
  // what a SAVE costs: the shadow's run-table bytes plus a compact JSON of the
  // deleted set and mark records. This is the honest durable-cost number; the
  // in-memory coordinate JSON (encodeState) is orders of magnitude above it on
  // chain-heavy docs.
  saveBytes(state) {
